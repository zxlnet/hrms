<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="atshfott.aspx.cs" Inherits="GotWell.HRMS.HRMSWeb.Views.atshfott.atshfott" %>
...
var atshfottConfig=Ext.decode('<%=ViewData["config"] %>'); 
atshfottConfig.auth=Ext.decode('<%=ViewData["authorization"] %>');
var atshfottPageName = 'atshfott';
var atshfottKeyColumn='sfcd';
            
var atshfottQueryWindow=function(grid){
	this.grid=grid;
	this.store=this.grid.getStore();
	this.init();	
	
	atshfottQueryWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:400, 
        height:270, 
        closeAction:'close', 
...
Ext.extend(atshfottQueryWindow,Ext.Window,{
    init:function(){
...
var atshfottEditWindow=function(grid,isNew){
	this.grid=grid;
	this.isNew=isNew||false;
	this.store=this.grid.getStore();
	this.init();	

	atshfottEditWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:650, 
        height:350, 
        ...
        listeners:{
            show:function(){              
		        var keyField = this.basisForm.findField(atshfottKeyColumn);
		        ...
		        {
		            keyField.setValue(atshfottConfig.sfcd);
		        }
		        var keyValue = keyField.getValue();
                setLastModifiedInfo(atshfottConfig,this.basisForm);
            },
            scope:this
        },
        buttons: [{ 
...
Ext.extend(atshfottEditWindow,Ext.Window,{
    init:function(){
		...
		return new Ext.FormPanel({   
	         ...
	         header:true,
      		 items: [
      		    ...
      		    items:[
                    {columnWidth:.5,layout: 'form',
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Attendance_Label_sfnm + '(<font color=red>*</font>)' ,
                        name: 'sfcd',stateful:false,typeAhead: true,allowBlank:false,
      		        ...
      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'timefield',fieldLabel:HRMSRes.Attendance_Label_otst + '(<font color=red>*</font>)' ,format:DATE_FORMAT.TIMENOSECOND,
                        name: 'otst',stateful:false,anchor:'95%',disabled:!this.isNew,allowBlank:false}]},

                    {columnWidth:.5,layout: 'form',
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Attendance_Label_otnm + '(<font color=red>*</font>)' ,
                        ...
                        store: new Ext.data.Store({ 
		                    ...
		                    listeners:{load:function(){var f =this.basisForm.findField('otcd') ;f.setValue(f.getValue());},scope:this}})
    		          }]},
	...
	save: function(){
        ...
        keyparams[1]={ColumnName:'otst',ColumnValue:this.basisForm.findField('otst').getValue(),ColumnType:'datetime'};
        
	    var method=this.isNew?'new':'edit';
		Ext.Ajax.request({
		   url: ContextInfo.contextPath+'/' + atshfottPageName + '.mvc/'+method,
...
var atshfottPanel=function(){
    this.tabId=atshfottConfig.tabId;
	this.init();	
	
	atshfottPanel.superclass.constructor.call(this,{
		applyTo:this.tabId, 
	    layout:'fit',
	    ...
	    items:[{
	        ...
	        tbar: [{ 
	            ...
	            hidden:atshfottConfig.auth[this.tabId+'_' + 'add']!='True',
	            handler: function(){
	                new atshfottEditWindow(this.grid,true).show();            	
	            }, 
	            scope: this 
	        },'-',{ 
	        	id:this.tabId+ '_edit',
	            ...
	            hidden:atshfottConfig.auth[this.tabId+'_' + 'edit']!='True',
	            handler: function(){
	            	new atshfottEditWindow(this.grid).show(); 
	            }, 
	            scope: this
	        },{ 
	            id:this.tabId+ '_delete',
                iconCls:'icon-remove', 
                hidden:atshfottConfig.auth[this.tabId+'_' + 'delete']!='True',
	            ...
	            iconCls:'icon-query', 
	            text:HRMSRes.Public_Toolbar_Query,
	            hidden:atshfottConfig.auth[this.tabId+'_' + 'query']!='True',
	            handler: function(){
	            	new atshfottQueryWindow(this.grid).show();
	            }, 
	            scope: this
	        },'-',{ 
	            ...
	            hidden:atshfottConfig.auth[this.tabId+'_' + 'exportexcel']!='True',
                ...
                id:this.tabId+'_muf',
                iconCls:atshfottConfig.muf=='True'?'icon-mufdelete':'icon-mufadd', 
                text:'', 
                handler: function(){updateMUF(this.tabId,atshfottConfig.muf?'delete':'add',atshfottConfig,this.grid);},
                scope: this 
            }], 
	        items:this.grid
       }]
...
Ext.extend(atshfottPanel,Ext.Panel,{
    init:function(){
	...
	createGridPanel:function(){
		...
		var atshfottStoreType=Ext.data.Record.create([
            {name:'sfcd'},
            {name:'sfnm'},
            {name:'otst'},
            {name:'otcd'},
		...
		var store=new Ext.data.Store({ 
        	reader: new Ext.data.JsonReader({
	   	    ...
	   	    baseParams:{record:Ext.encode({params:[{ColumnName:atshfottKeyColumn,ColumnValue:atshfottConfig.sfcd}]})},   
	   		url:ContextInfo.contextPath+'/' + atshfottPageName + '.mvc/list',
	   		listeners:{
        ...
        return new Ext.grid.GridPanel({
            ...
            cm: new Ext.grid.ColumnModel([ 
                ...
                {header:HRMSRes.Attendance_Label_otst,sortable: true, dataIndex: 'otst',renderer:formatTimeOnly},
	...
	remove:function(){
        ...
        keyparams[1]={ColumnName:'otst',ColumnValue:formatDateNoTime(record.get('otst')),ColumnType:'datetime'};
	    ...
	    Ext.MessageBox.show({
	        ...
	        fn: function(btn, text){
		        if (btn=='yes'){
			        Ext.Ajax.request({
	   			        url:ContextInfo.contextPath+ '/'+ atshfottPageName + '.mvc/delete',
	   			        success: function(response){
	   				        var o= Ext.util.JSON.decode(response.responseText);
	   				        if(o.status=='success'){
	...
	exportExcel:function(){
	    ...
	    var params={record:Ext.encode({params:[{ColumnName:atshfottKeyColumn,ColumnValue:''}],headers:header})};
	    ...
	    form.name='excelForm';
	    form.method='post';
	    form.action=ContextInfo.contextPath+ '/' + atshfottPageName + '.mvc/exportexcel';
	    for(var p in params){
