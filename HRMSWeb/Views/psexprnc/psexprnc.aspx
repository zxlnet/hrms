<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="psexprnc.aspx.cs" Inherits="GotWell.HRMS.HRMSWeb.Views.psexprnc.psexprnc" %>
...
var psexprncConfig=Ext.decode('<%=ViewData["config"] %>'); 
psexprncConfig.auth=Ext.decode('<%=ViewData["authorization"] %>');
var psexprncPageName = 'psexprnc';
var psexprncKeyColumn='emno';
            
var psexprncQueryWindow=function(grid){
	this.grid=grid;
	this.store=this.grid.getStore();
	this.init();	
	
	psexprncQueryWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:400, 
        height:350, 
        closeAction:'close', 
...
Ext.extend(psexprncQueryWindow,Ext.Window,{
    init:function(){
		...
		return new Ext.FormPanel({   
             ...
             items: [
                     ...
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Public_Label_sftn,
      		        ...
      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Personal_Label_cmnm,
                        name: 'cmnm',stateful:false,anchor:'95%'}]},
            
      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Personal_Label_emnm,
                        name: 'emnm',stateful:false,anchor:'95%'}]},

      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Personal_Label_jbds,
                        name: 'jbds',stateful:false,anchor:'95%'}]},
                    ...
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Master_Label_conm ,
                        ...
                        store: new Ext.data.Store({ 
		                    ...
		                    listeners:{load:function(){f = this.form.findField('trcd');f.setValue(f.getValue());},scope:this}})
    		          }]},
      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Personal_Label_fpos,
                        name: 'fpos',stateful:false,anchor:'95%'}]},
...
var psexprncEditWindow=function(grid,isNew){
	this.grid=grid;
	this.isNew=isNew||false;
	this.store=this.grid.getStore();
	this.init();	

	psexprncEditWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:650, 
        height:500, 
        ...
        listeners:{
            show:function(){              
		        var keyField = this.basisForm.findField(psexprncKeyColumn);
		        ...
		        {
		            keyField.setValue(psexprncConfig.emno);
		            if (psexprncConfig.emno!='')
		                getMaxsqno(psexprncConfig.emno,psexprncConfig.tableName,this.basisForm);
		        }
		        var keyValue = keyField.getValue();
                setLastModifiedInfo(psexprncConfig,this.basisForm);
            },
            scope:this
        },
        buttons: [{ 
...
Ext.extend(psexprncEditWindow,Ext.Window,{
    init:function(){
      		    ...
      		    items:[
                    ...
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Public_Label_staff + '(<font color=red>*</font>)',
                        ...
                        store: new Ext.data.Store({ 
                            ...
                            listeners:{select:function(p){var emno = p.getValue();if (emno!=''){getMaxsqno(emno,psexprncConfig.tableName,this.basisForm);}},scope:this}
    		          }]},
                     ...
                     items: [{xtype:'datefield',fieldLabel:HRMSRes.Personal_Label_todt+'(<font color=red>*</font>)',id:'todt',
      		        ...
      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Personal_Label_cmnm+'(<font color=red>*</font>)',
                        name: 'cmnm',stateful:false,anchor:'95%',allowBlank:false}]},
                     ...
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Personal_Label_emnm,
      		        ...
      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Personal_Label_fpos+'(<font color=red>*</font>)',
                        name: 'fpos',stateful:false,anchor:'95%',allowBlank:false}]},

      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Personal_Label_dept,
                        name: 'dept',stateful:false,anchor:'95%'}]},
                    ...
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Master_Label_trnm ,
                        ...
                        store: new Ext.data.Store({ 
		                    ...
		                    listeners:{load:function(){f = this.basisForm.findField('trcd');f.setValue(f.getValue());},scope:this}})
    		          }]},

                    {columnWidth:.5,layout: 'form',
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Master_Label_bnnm ,
                        name: 'bncd',stateful:false,typeAhead: true,
                        triggerAction: 'all',mode: 'local',maxHeight:150,anchor:'95%',
                        displayField: 'DisplayField',valueField:'ValueField',
                        store: new Ext.data.Store({ 
		                    reader: new Ext.data.JsonReader({totalProperty: "results",root: "rows",
			                    fields: ['ValueField','DisplayField']}), autoLoad:true,
		                    url:ContextInfo.contextPath+'/dropdown.mvc/getBusinessNature',
		                    listeners:{load:function(){f = this.basisForm.findField('bncd');f.setValue(f.getValue());},scope:this}})
    		          }]},
                    ...
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Master_Label_conm ,
      		        ...
      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Personal_Label_cmsc,
                        name: 'cmsc',stateful:false,anchor:'95%'}]},

      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Personal_Label_ctno,
                        name: 'ctno',stateful:false,anchor:'95%'}]},

      		        {columnWidth:1,layout: 'form',
                     items: [{xtype:'textarea',fieldLabel:HRMSRes.Personal_Label_jbds,height:40,
                        name: 'jbds',stateful:false,anchor:'98%'}]},

                    {columnWidth:.5,layout: 'form',
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Personal_Label_iscm ,
                        name: 'iscm',stateful:false,editable:false,typeAhead: true,
                        triggerAction: 'all',mode: 'local',maxHeight:150,anchor:'95%',
                        displayField: 'text',valueField:'value',
                        store: FlagYesNoStore}]},

      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'numberfield',fieldLabel:HRMSRes.Personal_Label_stsa,
                        name: 'stsa',stateful:false,anchor:'95%',keepZero:true}]},

      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'numberfield',fieldLabel:HRMSRes.Personal_Label_saly,
                        name: 'saly',stateful:false,anchor:'95%',keepZero:true}]},

      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'numberfield',fieldLabel:HRMSRes.Personal_Label_wkyr,
                        name: 'wkyr',stateful:false,anchor:'95%',keepZero:true}]},

      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Personal_Label_imbo,
                        name: 'imbo',stateful:false,anchor:'95%'}]},

      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Personal_Label_bott,
                        name: 'bott',stateful:false,anchor:'95%'}]},
	...
	save: function(){
		...
		Ext.Ajax.request({
		   url: ContextInfo.contextPath+'/' + psexprncPageName + '.mvc/'+method,
...
var psexprncPanel=function(){
    this.tabId=psexprncConfig.tabId;
	this.init();	
	
	psexprncPanel.superclass.constructor.call(this,{
		applyTo:this.tabId, 
	    layout:'fit',
	    ...
	    items:[{
	        ...
	        tbar: [{ 
	            ...
	            hidden:psexprncConfig.auth[this.tabId+'_' + 'add']!='True',
	            handler: function(){
	                new psexprncEditWindow(this.grid,true).show();            	
	            }, 
	            scope: this 
	        },'-',{ 
	        	id:this.tabId+ '_edit',
	            ...
	            hidden: psexprncConfig.auth[this.tabId + '_' + 'edit'] != 'True',
	            disabled: true,
	            handler: function(){
	            	new psexprncEditWindow(this.grid).show(); 
	            }, 
	            scope: this
	        },{ 
	            id:this.tabId+ '_delete',
                iconCls:'icon-remove',
                hidden: psexprncConfig.auth[this.tabId + '_' + 'delete'] != 'True',
                disabled: true,
	            ...
	            iconCls:'icon-query', 
	            text:HRMSRes.Public_Toolbar_Query,
	            hidden:psexprncConfig.auth[this.tabId+'_' + 'query']!='True',
	            handler: function(){
	            	new psexprncQueryWindow(this.grid).show();
	            }, 
	            scope: this
	        },'-',{ 
	            ...
	            hidden:psexprncConfig.auth[this.tabId+'_' + 'exportexcel']!='True',
	            handler: this.exportExcel, 
	            scope: this 
	        },'->',{ 
	        	xtype:'label', 
	            html:  '<b><font color=green>employee no: ' + psexprncConfig.emno + '</font></b>',
	            hidden: psexprncConfig.emno==''
	        },'->',{ 
                ...
                id:this.tabId+'_muf',
                iconCls:psexprncConfig.muf=='True'?'icon-mufdelete':'icon-mufadd', 
                text:'', 
                handler: function(){
                        updateMUF(this.tabId,psexprncConfig.muf?'delete':'add',psexprncConfig,this.grid);
                        },
                scope: this 
            }], 
	        items:this.grid
...
Ext.extend(psexprncPanel,Ext.Panel,{
    init:function(){
		this.grid=this.createGridPanel();
	},

	createGridPanel:function(){
		var psexprncStoreType=Ext.data.Record.create([
		    {name:'emno'},
            {name:'sfid'},
            {name:'stfn'},
            {name:'sqno'},
		...
		var store=new Ext.data.Store({ 
        	reader: new Ext.data.JsonReader({
	   	    ...
	   	    baseParams:{record:Ext.encode({params:[{ColumnName:psexprncKeyColumn,ColumnValue:psexprncConfig.emno}]})},   
	   		url:ContextInfo.contextPath+'/' + psexprncPageName + '.mvc/list',
	   		listeners:{
        ...
        return new Ext.grid.GridPanel({
            ...
            cm: new Ext.grid.ColumnModel([ 
                ...
                {header:HRMSRes.Personal_Label_cmnm,sortable: true, dataIndex: 'cmnm'},
                {header:HRMSRes.Personal_Label_emnm,sortable: true, dataIndex: 'emnm'},
                {header:HRMSRes.Personal_Label_fpos,sortable: true, dataIndex: 'fpos'},
                {header:HRMSRes.Master_Label_bnnm,sortable: true, dataIndex: 'bnnm'},
                {header:HRMSRes.Personal_Label_wkyr,sortable: true, dataIndex: 'wkyr'},
                {header:HRMSRes.Master_Label_trnm,sortable: true, dataIndex: 'trnm'},
	...
	remove:function(){
	    ...
	    Ext.MessageBox.show({
	        ...
	        fn: function(btn, text){
		        if (btn=='yes'){
			        Ext.Ajax.request({
	   			        url:ContextInfo.contextPath+ '/'+ psexprncPageName + '.mvc/delete',
	   			        success: function(response){
	   				        var o= Ext.util.JSON.decode(response.responseText);
	   				        if(o.status=='success'){
	...
	exportExcel:function(){
	    ...
	    var params={record:Ext.encode({params:[{ColumnName:psexprncKeyColumn,ColumnValue:''}],headers:header})};
	    ...
	    form.name='excelForm';
	    form.method='post';
	    form.action=ContextInfo.contextPath+ '/' + psexprncPageName + '.mvc/exportexcel';
	    for(var p in params){
