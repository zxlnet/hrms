<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="atoridat.aspx.cs" Inherits="GotWell.HRMS.HRMSWeb.Views.atoridat.atoridat" %>
...
var atoridatConfig=Ext.decode('<%=ViewData["config"] %>'); 
atoridatConfig.auth=Ext.decode('<%=ViewData["authorization"] %>');
var atoridatPageName = 'atoridat';
var atoridatKeyColumn='';
            
var atoridatQueryWindow=function(grid){
	this.grid=grid;
	this.store=this.grid.getStore();
	this.init();	
	
	atoridatQueryWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:400, 
        height:270, 
        closeAction:'close', 
...
Ext.extend(atoridatQueryWindow,Ext.Window,{
    init:function(){
		...
		return new Ext.FormPanel({   
             ...
             items: [
                     ...
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Public_Label_sftn,
      		        ...
      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'datefield',fieldLabel:HRMSRes.Attendance_Label_retm + '' + HRMSRes.Public_Label_From,id:'from|retm',
                        name:'from|retm',editable:false,height:22,anchor:'95%',
                        format: DATE_FORMAT.DATEONLY,minValue: '1980/01/01',
           	 	        invalidText : '{0} '+ HRMSRes.Public_Messsage_FormatInCorrect +' {1}'}]},

     		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'datefield',fieldLabel:HRMSRes.Attendance_Label_retm + '' + HRMSRes.Public_Label_To,id:'to|retm',
                        name:'to|retm',editable:false,height:22,anchor:'95%',
                        format: DATE_FORMAT.DATEONLY,minValue: '1980/01/01',
           	 	        invalidText : '{0} '+ HRMSRes.Public_Messsage_FormatInCorrect +' {1}'}]},
...
var atoridatEditWindow=function(grid,isNew){
	this.grid=grid;
	this.isNew=isNew||false;
	this.store=this.grid.getStore();
	this.init();	

	atoridatEditWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:650, 
        height:300, 
                ...
                if(!this.isNew){	
                    ...
                    this.basisForm.items.each(function(f){
                        if(f.isFormField){
                            ...
                            if (f.xtype=='datefield') value = formatDate(value);
                            f.setValue(value);                                               	            
                        }
                    });   
		        }
                setLastModifiedInfo(atoridatConfig,this.basisForm);
                this.oldretm = this.basisForm.findField('retm').getValue();
            },
            scope:this
        },
        buttons: [{ 
...
Ext.extend(atoridatEditWindow,Ext.Window,{
    init:function(){
...
//                        name: 'sfid',stateful:false,disabled:!this.isNew,allowBlank:false,typeAhead: true,
...
//      		        
       		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Public_Label_staff+ '(<font color=red>*</font>)',disabled:!this.isNew,
                        name: 'sfid',stateful:false,anchor:'95%',allowBlank:false}]},

     		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'datefield',fieldLabel:HRMSRes.Attendance_Label_retm + '(<font color=red>*</font>)',id:'retm',
                        name:'retm',editable:false,height:22,anchor:'95%',allowBlank:false,
                        format: DATE_FORMAT.DATETIME,minValue: '1980/01/01',stateful:false,disabled:false,
           	 	        invalidText : '{0} '+ HRMSRes.Public_Messsage_FormatInCorrect +' {1}'}]},
	...
	save: function(){
        ...
        var keyparams=[];
        keyparams[0]={ColumnName:'sfid',ColumnValue:this.basisForm.findField('sfid').getValue()};
        if (!this.isNew)
            keyparams[1]={ColumnName:'retm',ColumnValue:formatDateTime(this.oldretm),ColumnType:'datetime'};
        
	    var method=this.isNew?'new':'edit';
		Ext.Ajax.request({
		   url: ContextInfo.contextPath+'/' + atoridatPageName + '.mvc/'+method,
...
var atoridatPanel=function(){
    this.tabId=atoridatConfig.tabId;
	this.init();	
	
	atoridatPanel.superclass.constructor.call(this,{
		applyTo:this.tabId, 
	    layout:'fit',
	    ...
	    items:[{
	        ...
	        tbar: [{ 
	            ...
	            hidden:atoridatConfig.auth[this.tabId+'_' + 'add']!='True',
	            handler: function(){
	                new atoridatEditWindow(this.grid,true).show();            	
	            }, 
	            scope: this 
	        },'-',{ 
	        	id:this.tabId+ '_edit',
	            ...
	            hidden:atoridatConfig.auth[this.tabId+'_' + 'edit']!='True',
	            handler: function(){
	            	new atoridatEditWindow(this.grid).show(); 
	            }, 
	            scope: this
	        },{ 
	            id:this.tabId+ '_delete',
                iconCls:'icon-remove', 
                hidden:atoridatConfig.auth[this.tabId+'_' + 'delete']!='True',
	            ...
	            iconCls:'icon-query', 
	            text:HRMSRes.Public_Toolbar_Query,
	            hidden:atoridatConfig.auth[this.tabId+'_' + 'query']!='True',
	            handler: function(){
	            	new atoridatQueryWindow(this.grid).show();
	            }, 
	            scope: this
	        },'-',{ 
	            ...
	            hidden:atoridatConfig.auth[this.tabId+'_' + 'exportexcel']!='True',
	            handler: this.exportExcel, 
	            scope: this 
	        },'->',{ 
	        	xtype:'label', 
	            html:  '<b><font color=green>employee no: ' + atoridatConfig.emno + '</font></b>',
	            hidden: atoridatConfig.emno==''
	        },'->',{ 
                ...
                id:this.tabId+'_muf',
                iconCls:atoridatConfig.muf=='True'?'icon-mufdelete':'icon-mufadd', 
                text:'', 
                handler: function(){updateMUF(this.tabId,atoridatConfig.muf?'delete':'add',atoridatConfig,this.grid);},
                scope: this 
            }], 
	        items:this.grid
       }]
...
Ext.extend(atoridatPanel,Ext.Panel,{
    init:function(){
	...
	createGridPanel:function(){
		...
		var atoridatStoreType=Ext.data.Record.create([
		    //{name:'emno'},
            {name:'sfid'},
            {name:'stfn'},           
            {name:'retm'},                                      
		...
		var store=new Ext.data.Store({ 
        	reader: new Ext.data.JsonReader({
	   	    ...
	   	    baseParams:{record:Ext.encode({params:[{ColumnName:"emno",ColumnValue:atoridatConfig.emno},
	   	                                           {ColumnName:"from|retm",ColumnValue:atoridatConfig.atdt},
	   	                                           {ColumnName:"to|retm",ColumnValue:atoridatConfig.atdt}
	   	                                           ]})},   
	   		url:ContextInfo.contextPath+'/' + atoridatPageName + '.mvc/list',
	   		listeners:{
        ...
        return new Ext.grid.GridPanel({
            ...
            cm: new Ext.grid.ColumnModel([ 
                ...
                {header:HRMSRes.Attendance_Label_retm,sortable: true, dataIndex: 'retm',renderer:formatDate},
	...
	remove:function(){
		...
		var keyparams=[];
        keyparams[0]={ColumnName:'sfid',ColumnValue:record.get('sfid')};
        keyparams[1]={ColumnName:'retm',ColumnValue:formatDate(record.get('retm')),ColumnType:'datetime'};
	    ...
	    Ext.MessageBox.show({
	        ...
	        fn: function(btn, text){
		        if (btn=='yes'){
			        Ext.Ajax.request({
	   			        url:ContextInfo.contextPath+ '/'+ atoridatPageName + '.mvc/delete',
	   			        success: function(response){
	   				        var o= Ext.util.JSON.decode(response.responseText);
	   				        if(o.status=='success'){
	...
	exportExcel:function(){
	    ...
	    var params={record:Ext.encode({params:[{ColumnName:atoridatKeyColumn,ColumnValue:''}],headers:header})};
	    ...
	    form.name='excelForm';
	    form.method='post';
	    form.action=ContextInfo.contextPath+ '/' + atoridatPageName + '.mvc/exportexcel';
	    for(var p in params){
