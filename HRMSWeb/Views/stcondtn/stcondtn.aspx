<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="stcondtn.aspx.cs" Inherits="GotWell.HRMS.HRMSWeb.Views.stcondtn.stcondtn" %>
...
var stcondtnConfig=Ext.decode('<%=ViewData["config"] %>'); 
stcondtnConfig.auth=Ext.decode('<%=ViewData["authorization"] %>');
var stcondtnPageName = 'stcondtn';
var stcondtnKeyColumn='cdno';
            
var stcondtnQueryWindow=function(grid){
	this.grid=grid;
	this.store=this.grid.getStore();
	this.init();	
	
	stcondtnQueryWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:400, 
        height:300, 
        closeAction:'close', 
...
Ext.extend(stcondtnQueryWindow,Ext.Window,{
    init:function(){
	...
	createFormPanel:function(){	        
		return new Ext.FormPanel({   
             ...
             items: [
                     ...
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.System_Label_cdno,
                        name: 'cdno',stateful:false,anchor:'95%',UpperOnBlur:'true'}]},

      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.System_Label_cdnm,
                        name: 'cdnm',stateful:false,anchor:'95%'}]},

      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.System_Label_ownr,
...
var stcondtnEditWindow=function(grid,isNew){
	this.grid=grid;
	...
	stcondtnEditWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:650, 
        height:350, 
        ...
        listeners:{
            show:function(){              
		        var keyField = this.basisForm.findField(stcondtnKeyColumn);
                ...
                if(!this.isNew){	
                    ...
                    this.formCdtx.items.each(function(f){
                        if(f.isFormField){
                            var value=data.get(f.getName());
                            if (f.xtype=='timefield') value = formatTime(value);
                            if (f.xtype=='datefield') value = formatDateNoTime(value);
                ...
                setLastModifiedInfo(stcondtnConfig,this.basisForm);
            },
            scope:this
        },
        buttons: [{ 
...
Ext.extend(stcondtnEditWindow,Ext.Window,{
    init:function(){
		this.basisFormPanel=this.createBasisFormPanel();
		this.basisForm=this.basisFormPanel.getForm();
		
		this.formCdtxPanel=this.createCdtxFormPanel();
		this.formCdtx=this.formCdtxPanel.getForm();
		...
		return new Ext.FormPanel({   
	         ...
	         header:true,
     		 items: [
      		    ...
      		    items:[
      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.System_Label_cdno + '(<font color=red>*</font>)',
                        name: 'cdno',stateful:false,anchor:'95%',disabled:!this.isNew,allowBlank:false}]},

      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.System_Label_cdnm + '(<font color=red>*</font>)',
                        name: 'cdnm',stateful:false,anchor:'95%',allowBlank:false}]},

       		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.System_Label_dscr,
                        name: 'dscr',stateful:false,anchor:'95%'}]},
                                       
                    {columnWidth:.5,layout: 'form',
                    items: [{xtype:'combo',fieldLabel:HRMSRes.System_Label_ownr ,
                        name: 'ownr',stateful:false,typeAhead: true,
                        ...
                        store: new Ext.data.Store({ 
		                    ...
		                    listeners:{
                                load:function(){var f = this.basisForm.findField('ownr');f.setValue(f.getValue());},
                                scope:this}}),
		...
		return new Ext.FormPanel({   
             ...
             items: [
                     ...
                     items: [{xtype:'textarea',fieldLabel:HRMSRes.Payroll_Label_cdtx,
                        id:'cdtx',stateful:false,anchor:'95%',height:130,
	...
	save: function(){
        ...
        var keyparams=[];
        keyparams[0]={ColumnName:'cdno',ColumnValue:this.basisForm.findField('cdno').getValue()};
        
	    var method=this.isNew?'new':'edit';
		Ext.Ajax.request({
		   url: ContextInfo.contextPath+'/' + stcondtnPageName + '.mvc/'+method,
...
var stcondtnPanel=function(){
    this.tabId=stcondtnConfig.tabId;
	this.init();	
	
	stcondtnPanel.superclass.constructor.call(this,{
		applyTo:this.tabId, 
	    layout:'fit',
	    ...
	    items:[{
	        ...
	        tbar: [{ 
	            ...
	            hidden:stcondtnConfig.auth[this.tabId+'_' + 'add']!='True',
	            handler: function(){
	                new stcondtnEditWindow(this.grid,true).show();            	
	            }, 
	            scope: this 
	        },'-',{ 
	        	id:this.tabId+ '_edit',
	            ...
	            hidden:stcondtnConfig.auth[this.tabId+'_' + 'edit']!='True',
	            handler: function(){
	            	new stcondtnEditWindow(this.grid).show(); 
	            }, 
	            scope: this
	        },{ 
	            id:this.tabId+ '_delete',
                iconCls:'icon-remove', 
                hidden:stcondtnConfig.auth[this.tabId+'_' + 'delete']!='True',
	            ...
	            iconCls:'icon-query', 
	            text:HRMSRes.Public_Toolbar_Query,
	            hidden:stcondtnConfig.auth[this.tabId+'_' + 'query']!='True',
	            handler: function(){
	            	new stcondtnQueryWindow(this.grid).show();
	            }, 
	            scope: this
	        },'-',{ 
	            ...
	            hidden:stcondtnConfig.auth[this.tabId+'_' + 'exportexcel']!='True',
	            handler: this.exportExcel, 
	            scope: this 
	        },'->',{ 
                id:this.tabId+'_muf',
                iconCls:stcondtnConfig.muf=='True'?'icon-mufdelete':'icon-mufadd', 
                text:'', 
                handler: function(){
                        updateMUF(this.tabId,stcondtnConfig.muf=='True'?'delete':'add',stcondtnConfig,this.grid);
                        var f = Ext.getCmp(this.tabId+'_muf');
                        if (stcondtnConfig.muf=='True')
                            f.iconCls='icon-mufdelete';
                        else
                            f.iconCls='icon-mufadd';
                        },
                scope: this 
            }], 
	        items:this.grid
...
Ext.extend(stcondtnPanel,Ext.Panel,{
    init:function(){
	...
	createGridPanel:function(){
		...
		var stcondtnStoreType=Ext.data.Record.create([
		    {name:'emno'},{name:'sfid'},{name:'stfn'},{name:'cdno'},{name:'cdnm'},{name:'dscr'},
            {name:'cdtx'},{name:'ownr'},{name:'lmtm'},
            {name:'lmur'}
		]);
		...
		var store=new Ext.data.Store({ 
        	reader: new Ext.data.JsonReader({
	   	    ...
	   	    baseParams:{record:Ext.encode({params:[{ColumnName:stcondtnKeyColumn,ColumnValue:''}]})},   
	   		url:ContextInfo.contextPath+'/' + stcondtnPageName + '.mvc/list',
	   		listeners:{
        ...
        return new Ext.grid.GridPanel({
            ...
            cm: new Ext.grid.ColumnModel([ 
                {header:HRMSRes.System_Label_cdno,sortable: true, dataIndex: 'cdno'},
                {header:HRMSRes.System_Label_cdnm,sortable: true, dataIndex: 'cdnm'},
                {header:HRMSRes.System_Label_dscr,sortable: true, dataIndex: 'dscr'},
                {header:HRMSRes.System_Label_cdtx,sortable: true, dataIndex: 'cdtx'},
                {header:HRMSRes.System_Label_ownr,sortable: true, dataIndex: 'ownr'},
	...
	remove:function(){
		...
		var keyparams=[];
        keyparams[0]={ColumnName:'cdno',ColumnValue:record.get('cdno')};
	    ...
	    Ext.MessageBox.show({
	        ...
	        fn: function(btn, text){
		        if (btn=='yes'){
			        Ext.Ajax.request({
	   			        url:ContextInfo.contextPath+ '/'+ stcondtnPageName + '.mvc/delete',
	   			        success: function(response){
	   				        var o= Ext.util.JSON.decode(response.responseText);
	   				        if(o.status=='success'){
	...
	exportExcel:function(){
	    ...
	    var params={record:Ext.encode({params:[{ColumnName:stcondtnKeyColumn,ColumnValue:''}],headers:header})};
	    ...
	    form.name='excelForm';
	    form.method='post';
	    form.action=ContextInfo.contextPath+ '/' + stcondtnPageName + '.mvc/exportexcel';
	    for(var p in params){
