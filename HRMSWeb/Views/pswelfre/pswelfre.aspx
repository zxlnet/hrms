<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="pswelfre.aspx.cs" Inherits="GotWell.HRMS.HRMSWeb.Views.pswelfre.pswelfre" %>
...
var pswelfreConfig=Ext.decode('<%=ViewData["config"] %>'); 
pswelfreConfig.auth=Ext.decode('<%=ViewData["authorization"] %>');
var pswelfrePageName = 'pswelfre';
var pswelfreKeyColumn='emno';
            
var pswelfreQueryWindow=function(grid){
	this.grid=grid;
	this.store=this.grid.getStore();
	this.init();	
	
	pswelfreQueryWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:400, 
        height:320, 
        closeAction:'close', 
...
Ext.extend(pswelfreQueryWindow,Ext.Window,{
    init:function(){
	...
	createFormPanel:function(){	        
		return new Ext.FormPanel({   
             ...
             items: [
                    ...
                    {columnWidth:.5,layout: 'form',
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Master_Label_wtnm ,
                        name: 'wtcd',stateful:false,typeAhead: true,
                        ...
                        store: new Ext.data.Store({ 
		                    ...
		                    url:ContextInfo.contextPath+'/dropdown.mvc/getWelfareType',
		                    listeners:{load:function(){f = this.form.findField('wtcd');f.setValue(f.getValue());},scope:this}})
    		          }]},
                      
     		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'datefield',fieldLabel:HRMSRes.Public_Label_isdt + '' + HRMSRes.Public_Label_From,id:'from|isdt',
                        name:'from|isdt',editable:false,height:22,anchor:'95%',
                        format: DATE_FORMAT.DATEONLY,minValue: '1980/01/01',
           	 	        invalidText : '{0} '+ HRMSRes.Public_Messsage_FormatInCorrect +' {1}'}]},

     		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'datefield',fieldLabel:HRMSRes.Public_Label_isdt + '' + HRMSRes.Public_Label_To,id:'to|isdt',
                        name:'to|isdt',editable:false,height:22,anchor:'95%',
                        format: DATE_FORMAT.DATEONLY,minValue: '1980/01/01',
           	 	        invalidText : '{0} '+ HRMSRes.Public_Messsage_FormatInCorrect +' {1}'}]},
...
var pswelfreEditWindow=function(grid,isNew){
	this.grid=grid;
	this.isNew=isNew||false;
	this.store=this.grid.getStore();
	this.init();	

	pswelfreEditWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:650, 
        height:350, 
        ...
        listeners:{
            show:function(){              
		        var keyField = this.basisForm.findField(pswelfreKeyColumn);
		        ...
		        {
		            keyField.setValue(pswelfreConfig.emno);
		            if (pswelfreConfig.emno!='')
		                getMaxsqno(pswelfreConfig.emno,pswelfreConfig.tableName,this.basisForm);
		        }
		        var keyValue = keyField.getValue();
                setLastModifiedInfo(pswelfreConfig,this.basisForm);
            },
            scope:this
        },
        buttons: [{ 
...
Ext.extend(pswelfreEditWindow,Ext.Window,{
    init:function(){
      		    ...
      		    items:[
                    ...
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Public_Label_staff + '(<font color=red>*</font>)',
                        ...
                        store: new Ext.data.Store({ 
                            ...
                            listeners:{select:function(p){var emno = p.getValue();if (emno!=''){getMaxsqno(emno,pswelfreConfig.tableName,this.basisForm);}},scope:this}
    		          }]},
                    ...
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Master_Label_wtnm ,
                        ...
                        store: new Ext.data.Store({ 
		                    reader: new Ext.data.JsonReader({totalProperty: "results",root: "rows",
			                    fields: ['ValueField','DisplayField']}), autoLoad:true,
		                    url:ContextInfo.contextPath+'/dropdown.mvc/getWelfareType',
		                    listeners:{load:function(){f = this.basisForm.findField('wtcd');f.setValue(f.getValue());},scope:this}})
    		          }]},

     		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'datefield',fieldLabel:HRMSRes.Public_Label_isdt+'(<font color=red>*</font>)',id:'issuedate',
	...
	save: function(){
		...
		Ext.Ajax.request({
		   url: ContextInfo.contextPath+'/' + pswelfrePageName + '.mvc/'+method,
...
var pswelfrePanel=function(){
    this.tabId=pswelfreConfig.tabId;
	this.init();	
	
	var relatedFuncMenu = new Ext.menu.Menu({
    id: 'relatedFuncMenu',
    items: [
        {text: HRMSRes.Public_Menu_bswlftyp,
         handler: function() {mainPanel.loadClass(ContextInfo.contextPath+'/masterdata.mvc/index?tableName=tbswlftyp&menuId=M1610&helpId=bswlftyp','M1610','',HRMSRes.Public_Menu_bswlftyp,'','true');}}       
    ]});
    
	pswelfrePanel.superclass.constructor.call(this,{
		applyTo:this.tabId, 
	    layout:'fit',
	    ...
	    items:[{
	        ...
	        tbar: [{ 
	            ...
	            hidden:pswelfreConfig.auth[this.tabId+'_' + 'add']!='True',
	            handler: function(){
	                new pswelfreEditWindow(this.grid,true).show();            	
	            }, 
	            scope: this 
	        },'-',{ 
	        	id:this.tabId+ '_edit',
	            ...
	            hidden: pswelfreConfig.auth[this.tabId + '_' + 'edit'] != 'True',
	            disabled: true,
	            handler: function(){
	            	new pswelfreEditWindow(this.grid).show(); 
	            }, 
	            scope: this
	        },{ 
	            id:this.tabId+ '_delete',
                iconCls:'icon-remove',
                hidden: pswelfreConfig.auth[this.tabId + '_' + 'delete'] != 'True',
                disabled: true,
	            ...
	            iconCls:'icon-query', 
	            text:HRMSRes.Public_Toolbar_Query,
	            hidden:pswelfreConfig.auth[this.tabId+'_' + 'query']!='True',
	            handler: function(){
	            	new pswelfreQueryWindow(this.grid).show();
	            }, 
	            scope: this
	        },'-',{ 
	            ...
	            hidden:pswelfreConfig.auth[this.tabId+'_' + 'exportexcel']!='True',
	        	...
	        	xtype:'label', 
	            html:  '<b><font color=green>employee no: ' + pswelfreConfig.emno + '</font></b>',
	            hidden: pswelfreConfig.emno==''
	        },'->',{ 
                ...
                id:this.tabId+'_muf',
                iconCls:pswelfreConfig.muf=='True'?'icon-mufdelete':'icon-mufadd', 
                text:'', 
                handler: function(){updateMUF(this.tabId,pswelfreConfig.muf?'delete':'add',pswelfreConfig,this.grid);},
                scope: this 
            }], 
	        items:this.grid
       }]
...
Ext.extend(pswelfrePanel,Ext.Panel,{
    init:function(){
	...
	createGridPanel:function(){
		...
		var pswelfreStoreType=Ext.data.Record.create([
		    {name:'emno'},
            {name:'sfid'},
            {name:'stfn'},
            {name:'sqno'},
		...
		var store=new Ext.data.Store({ 
        	reader: new Ext.data.JsonReader({
	   	    ...
	   	    baseParams:{record:Ext.encode({params:[{ColumnName:pswelfreKeyColumn,ColumnValue:pswelfreConfig.emno}]})},   
	   		url:ContextInfo.contextPath+'/' + pswelfrePageName + '.mvc/list',
	   		listeners:{
        ...
        return new Ext.grid.GridPanel({
            ...
            cm: new Ext.grid.ColumnModel([ 
               ...
               // {header:HRMSRes.Public_Label_sqno,sortable: true, dataIndex: 'sqno'},
                ...
                {header:HRMSRes.Master_Label_wtnm,sortable: true, dataIndex: 'wtnm'},
	...
	remove:function(){
	    ...
	    Ext.MessageBox.show({
	        ...
	        fn: function(btn, text){
		        if (btn=='yes'){
			        Ext.Ajax.request({
	   			        url:ContextInfo.contextPath+ '/'+ pswelfrePageName + '.mvc/delete',
	   			        success: function(response){
	   				        var o= Ext.util.JSON.decode(response.responseText);
	   				        if(o.status=='success'){
	...
	exportExcel:function(){
	    ...
	    var params={record:Ext.encode({params:[{ColumnName:pswelfreKeyColumn,ColumnValue:''}],headers:header})};
	    ...
	    form.name='excelForm';
	    form.method='post';
	    form.action=ContextInfo.contextPath+ '/' + pswelfrePageName + '.mvc/exportexcel';
	    for(var p in params){
