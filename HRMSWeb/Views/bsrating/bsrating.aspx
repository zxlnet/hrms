<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="bsrating.aspx.cs" Inherits="GotWell.HRMS.HRMSWeb.Views.bsrating.bsrating" %>
...
var bsratingConfig=Ext.decode('<%=ViewData["config"] %>'); 
bsratingConfig.auth=Ext.decode('<%=ViewData["authorization"] %>');
var bsratingPageName = 'bsrating';
var bsratingKeyColumn='racd';
            
var bsratingQueryWindow=function(grid){
	this.grid=grid;
	this.store=this.grid.getStore();
	this.init();	
	
	bsratingQueryWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:400, 
        height:270, 
        closeAction:'close', 
...
Ext.extend(bsratingQueryWindow,Ext.Window,{
    init:function(){
	...
	createFormPanel:function(){	        
		return new Ext.FormPanel({   
             ...
             items: [
                     ...
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Master_Label_racd,
                        name: 'racd',stateful:false,anchor:'95%'}]},

      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Master_Label_ranm,
                        name: 'ranm',stateful:false,anchor:'95%'}]},
...
var bsratingEditWindow=function(grid,isNew){
	this.grid=grid;
	...
	bsratingEditWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:650, 
        height:445, 
        ...
        listeners:{
            show:function(){    
		        var keyField = this.basisForm.findField(bsratingKeyColumn);
                if(!this.isNew){	
                ...
                setLastModifiedInfo(bsratingConfig,this.basisForm);
...
Ext.extend(bsratingEditWindow,Ext.Window,{
    init:function(){
	         ...
	         autoHeight:true,
      		 items: [
      		    ...
      		    items:[
      		        ...
      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Master_Label_racd+'(<font color=red>*</font>)',
                        name: 'racd',allowBlank:false,stateful:false,anchor:'95%',disabled:!this.isNew}]},
     		        
      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Master_Label_ranm+'(<font color=red>*</font>)',
                        name: 'ranm',allowBlank:false,stateful:false,anchor:'95%'}]},
    ...
    createGridPanel:function(){
		...
		this.bsratingEditDtlStoreType=Ext.data.Record.create([
            {name:'rtcd'},
            {name:'racd'},
            {name:'rtnm'},
            {name:'remk'},
		...
		var store=new Ext.data.Store({ 
        	reader: new Ext.data.JsonReader({
	    		...
	    		root: "rows"               
	   	 	},this.bsratingEditDtlStoreType), 	
	   	    baseParams:{record:Ext.encode({params:[{ColumnName:bsratingKeyColumn,ColumnValue:''}]})},   
	   		url:ContextInfo.contextPath+'/' + bsratingPageName + '.mvc/getRatingDtl',
	   		//autoLoad:true,
	   		listeners:{
        ...
        return new Ext.grid.EditorGridPanel({
            id:'dtlGrid',
    		border:true, height:248,monitorResize:false, loadMask:true,ds: store, frame:false,collapsible: true,
            ...
            cm: new Ext.grid.ColumnModel([ 
                {header:HRMSRes.Master_Label_ricd,sortable: true, dataIndex: 'rtcd', editor: new Ext.form.TextField({})},
                {id:'rtnm',header: HRMSRes.Master_Label_rinm,dataIndex: 'rtnm', editor: new Ext.form.TextField({})},
	...
	save: function(){
        ...
        for (var i =0; i< sm.length;i++ ){
            var x = sm[i];
            var p = {racd:this.basisForm.findField('racd').getValue(),
                     rtcd:x.get('rtcd'),
                     rtnm:x.get('rtnm'),remk:x.get('remk')};

            dtlparams[dtlparams.length] = p;
        };
        
        var keyparams=[];
        keyparams[0]={ColumnName:'racd',ColumnValue:this.basisForm.findField('racd').getValue()};
        
	    var method=this.isNew?'new':'edit';
		Ext.Ajax.request({
		   url: ContextInfo.contextPath+'/' + bsratingPageName + '.mvc/'+method,
    ...
    query:function(){	    
        var params=[];	
        var f = this.basisForm.findField(bsratingKeyColumn);        
        var p={
            ColumnName:f.getName(),
            ColumnValue:f.getValue()                
        };
    ...
    addline:function(){	    
        ...
        var params=[];
        var v = this.basisForm.findField('racd').getValue();
        params['rtcd']= (v==''?n.toString() :this.basisForm.findField('racd').getValue() + '-' + n.toString());
        params['rtnm']= ''
        params['remk']='';
    ...
    deleteline:function(){
        ...
        if (sm.hasSelection())
        {
            ...
            this.dtldeletedline[this.dtldeletedline.length]=record.get('rtcd');
    ...
    exportExcelDtl:function(){
        ...
        var params={record:Ext.encode({params:[{ColumnName:bsratingKeyColumn,ColumnValue:''}],headers:header})};
        ...
        form.name='excelForm';
        form.method='post';
        form.action=ContextInfo.contextPath+ '/' + bsratingPageName + '.mvc/exportexceldtl';
        for(var p in params){
...
var bsratingPanel=function(){
    this.tabId=bsratingConfig.tabId;
	this.init();	
	
	bsratingPanel.superclass.constructor.call(this,{
		applyTo:this.tabId, 
	    layout:'fit',
	    ...
	    items:[{
	        ...
	        tbar: [{ 
	            ...
	            iconCls:'icon-add', 
	            text: HRMSRes.Public_Toolbar_Add, 
	            hidden:bsratingConfig.auth[this.tabId+'_' + 'add']!='True',
	            handler: function(){
	                new bsratingEditWindow(this.grid,true).show();            	
	            }, 
	            scope: this 
	        },'-',{ 
	        	id:this.tabId+ '_edit',
	            ...
	            hidden: bsratingConfig.auth[this.tabId + '_' + 'edit'] != 'True',
	            disabled: true,
	            handler: function(){
	            	new bsratingEditWindow(this.grid).show(); 
	            }, 
	            scope: this
	        },{ 
	            id:this.tabId+ '_delete',
                iconCls:'icon-remove',
                hidden: bsratingConfig.auth[this.tabId + '_' + 'delete'] != 'True',
                disabled: true,
                text:HRMSRes.Public_Toolbar_Delete, 
                handler: this.remove, 
                scope: this 
	            ...
	            iconCls:'icon-query', 
	            text:HRMSRes.Public_Toolbar_Query,
	            hidden:bsratingConfig.auth[this.tabId+'_' + 'query']!='True',
	            handler: function(){
	            	new bsratingQueryWindow(this.grid).show();
	            }, 
	            scope: this
	        },'-',{ 
	            ...
	            iconCls:'icon-export', 
	            text:HRMSRes.Public_Toolbar_ToExcel, 
	            hidden:bsratingConfig.auth[this.tabId+'_' + 'exportexcel']!='True',
                ...
                id:this.tabId+'_muf',
                iconCls:bsratingConfig.muf=='True'?'icon-mufdelete':'icon-mufadd', 
                text:'', 
                handler: function(){updateMUF(this.tabId,bsratingConfig.muf?'delete':'add',bsratingConfig,this.grid);},
                scope: this 
            }], 
	        items:this.grid
       }]
...
Ext.extend(bsratingPanel,Ext.Panel,{
    init:function(){
	...
	createGridPanel:function(){
		...
		var bsratingStoreType=Ext.data.Record.create([
		    {name:'racd'},{name:'ranm'},{name:'remk'},{name:'lmtm'},
            {name:'lmur'},
            {name:'rfid'}
		]);
		...
		var store=new Ext.data.Store({ 
        	reader: new Ext.data.JsonReader({
	   	    ...
	   	    baseParams:{record:Ext.encode({params:[{ColumnName:bsratingKeyColumn,ColumnValue:bsratingConfig.emno}]})},   
	   		url:ContextInfo.contextPath+'/' + bsratingPageName + '.mvc/list',
	   		listeners:{
                loadexception:function(o,t,response)
                {var o= Ext.util.JSON.decode(response.responseText);		   		
                this.grid.getBottomToolbar().diplayMsg.update(o.msg);},
        ...
        return new Ext.grid.GridPanel({
            ...
            cm: new Ext.grid.ColumnModel([ 
                {header:HRMSRes.Master_Label_racd,sortable: true, dataIndex: 'racd'},
                {header:HRMSRes.Master_Label_ranm,sortable: true, dataIndex: 'ranm'},
	...
	remove:function(){
		...
		var keyparams=[];
        keyparams[0]={ColumnName:'racd',ColumnValue:record.get('racd')};
	    ...
	    Ext.MessageBox.show({
	        ...
	        fn: function(btn, text){
		        if (btn=='yes'){
			        Ext.Ajax.request({
	   			        url:ContextInfo.contextPath+ '/'+ bsratingPageName + '.mvc/delete',
	   			        success: function(response){
	   				        var o= Ext.util.JSON.decode(response.responseText);
	   				        if(o.status=='success'){
	...
	exportExcel:function(){
	    ...
	    form.name='excelForm';
	    form.method='post';
	    form.action=ContextInfo.contextPath+ '/' + bsratingPageName + '.mvc/exportexcel';
	    for(var p in params){
	        var hd = document.createElement('input');
            hd.type = 'hidden';
            hd.name = p;
