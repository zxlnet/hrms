<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="prcumitm.aspx.cs" Inherits="GotWell.HRMS.HRMSWeb.Views.prcumitm.prcumitm" %>
...
var prcumitmConfig=Ext.decode('<%=ViewData["config"] %>'); 
prcumitmConfig.auth=Ext.decode('<%=ViewData["authorization"] %>');
var prcumitmPageName = 'prcumitm';
var prcumitmKeyColumn='emno';
            
var prcumitmQueryWindow=function(grid){
	this.grid=grid;
	this.store=this.grid.getStore();
	this.init();	
	
	prcumitmQueryWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:400, 
        height:200, 
        closeAction:'close', 
...
Ext.extend(prcumitmQueryWindow,Ext.Window,{
    init:function(){
...
Ext.extend(calculationStatusWindow,Ext.Window,{
	...
	calculation:function(){
		Ext.Ajax.request({
		   url: ContextInfo.contextPath+'/' + prcumitmPageName + '.mvc/calculation',
...
var prcumitmPanel=function(){
    this.tabId=prcumitmConfig.tabId;
	this.init();	
	
	prcumitmPanel.superclass.constructor.call(this,{
		applyTo:this.tabId, 
	    layout:'fit',
	    ...
	    items:[{
	        ...
	        tbar: [{ 
	            ...
	            //hidden:prcumitmConfig.auth[this.tabId+'_' + 'calculate']!='True',
	            handler: function(){
	            ...
	            iconCls:'icon-query', 
	            text:HRMSRes.Public_Toolbar_Query,
	            hidden:prcumitmConfig.auth[this.tabId+'_' + 'query']!='True',
	            handler: function(){
	            	new prcumitmQueryWindow(this.grid).show();
	            }, 
	            scope: this
	        },'-',{ 
	            ...
	            hidden:prcumitmConfig.auth[this.tabId+'_' + 'exportexcel']!='True',
                ...
                id:this.tabId+'_muf',
                iconCls:prcumitmConfig.muf=='True'?'icon-mufdelete':'icon-mufadd', 
                text:'', 
                handler: function(){updateMUF(this.tabId,prcumitmConfig.muf?'delete':'add',prcumitmConfig,this.grid);},
                scope: this 
            }], 
	        items:this.grid
       }]
...
Ext.extend(prcumitmPanel,Ext.Panel,{
    init:function(){
	...
	createGridPanel:function(){
	    ...
	    var prcumitmStoreType=Ext.data.Record.create([
		    {name:'emno'},
		    {name:'sfid'},
		    {name:'sfnm'},
		    {name:'perd'},
		...
		var store=new Ext.data.Store({ 
        	reader: new Ext.data.JsonReader({
	   	    ...
	   	    baseParams:{record:Ext.encode({params:[{ColumnName:prcumitmKeyColumn,ColumnValue:prcumitmConfig.emno}]})},   
	   		url:ContextInfo.contextPath+'/' + prcumitmPageName + '.mvc/listdetails',
	   		listeners:{
	...
	exportExcel:function(){
	    ...
	    var params={record:Ext.encode({params:[{ColumnName:prcumitmKeyColumn,ColumnValue:''}],headers:header})};
	    ...
	    form.name='excelForm';
	    form.method='post';
	    form.action=ContextInfo.contextPath+ '/' + prcumitmPageName + '.mvc/exportexcel';
	    for(var p in params){
