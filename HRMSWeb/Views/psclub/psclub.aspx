<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="psclub.aspx.cs" Inherits="GotWell.HRMS.HRMSWeb.Views.psclub.psclub" %>
...
var psclubConfig=Ext.decode('<%=ViewData["config"] %>'); 
psclubConfig.auth=Ext.decode('<%=ViewData["authorization"] %>');
var psclubPageName = 'psclub';
var psclubKeyColumn='emno';
            
var psclubQueryWindow=function(grid){
	this.grid=grid;
	this.store=this.grid.getStore();
	this.init();	
	
	psclubQueryWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:400, 
        height:290, 
        closeAction:'close', 
...
Ext.extend(psclubQueryWindow,Ext.Window,{
    init:function(){
	...
	createFormPanel:function(){	        
		return new Ext.FormPanel({   
             ...
             items: [
                    ...
                    {columnWidth:.5,layout: 'form',
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Master_Label_clnm,
                        name: 'clcb',stateful:false,typeAhead: true,
                        ...
                        store: new Ext.data.Store({ 
		                    ...
		                    url:ContextInfo.contextPath+'/dropdown.mvc/getClub',
		                    listeners:{load:function(){var f = this.form.findField('clcb');f.setValue(f.getValue());},scope:this}})
    		          }]},
            
      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Personal_Label_mbty,
                        name: 'mbty',stateful:false,anchor:'95%'}]},
...
var psclubEditWindow=function(grid,isNew){
	this.grid=grid;
	this.isNew=isNew||false;
	this.store=this.grid.getStore();
	this.init();	
                      
	psclubEditWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:650, 
        height:350, 
        ...
        listeners:{
            show:function(){              
		        var keyField = this.basisForm.findField(psclubKeyColumn);
		        ...
		        {
		            keyField.setValue(psclubConfig.emno);
		            if (psclubConfig.emno!='')
		                getMaxsqno(psclubConfig.emno,psclubConfig.tableName,this.basisForm);
		        }
		        var keyValue = keyField.getValue();
                setLastModifiedInfo(psclubConfig,this.basisForm);
            },
            scope:this
        },
        buttons: [{ 
...
Ext.extend(psclubEditWindow,Ext.Window,{
    init:function(){
      		    ...
      		    items:[
                    ...
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Public_Label_staff + '(<font color=red>*</font>)',
                        ...
                        store: new Ext.data.Store({ 
                            ...
                            listeners:{select:function(p){var emno = p.getValue();if (emno!='') {getMaxsqno(emno,psclubConfig.tableName,this.basisForm);}},scope:this}
    		          }]},
                    ...
                    {columnWidth:.5,layout: 'form',
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Master_Label_clnm+'(<font color=red>*</font>)' ,
                        name: 'clcd',stateful:false,typeAhead: true,
                        //disabled:psclubConfig.openMode=='A',
                        triggerAction: 'all',mode: 'local',maxHeight:150,anchor:'95%',
                        displayField: 'DisplayField',valueField:'ValueField',
                        store: new Ext.data.Store({ 
		                    reader: new Ext.data.JsonReader({totalProperty: "results",root: "rows",
			                    fields: ['ValueField','DisplayField']}), autoLoad:true,
		                    url:ContextInfo.contextPath+'/dropdown.mvc/getClub',
		                    listeners:{load:function(){var f = this.basisForm.findField('clcd');f.setValue(f.getValue());},scope:this}})
    		          }]},

      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Personal_Label_clna,
                        //disabled:psclubConfig.openMode=='A',
                        name: 'clna',stateful:false,anchor:'95%'}]},
                     ...
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Personal_Label_mbty,
      		        ...
      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Personal_Label_cmfe,
                        //disabled:psclubConfig.openMode=='A',
                        name: 'cmfe',stateful:false,anchor:'95%'}]},
                    ...
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Master_Label_crnm ,
                        ...
                        store: new Ext.data.Store({ 
		                    ...
		                    listeners:{load:function(){var f = this.basisForm.findField('crcd');f.setValue(f.getValue());},scope:this}})
    		          }]},

     		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'datefield',fieldLabel:HRMSRes.Personal_Label_jodt,id:'jodt',
                        name:'jodt',editable:false,height:22,anchor:'95%',
                        //disabled:psclubConfig.openMode=='A',
                     ...
                     items: [{xtype:'datefield',fieldLabel:HRMSRes.Public_Label_exdt,id:'exdt',
       		        ...
       		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Personal_Label_ssfe,
                        //disabled:psclubConfig.openMode=='A',
                        name: 'ssfe',stateful:false,anchor:'95%'}]},
	...
	save: function(){
		...
		Ext.Ajax.request({
		   url: ContextInfo.contextPath+'/' + psclubPageName + '.mvc/'+method,
...
var psclubPanel=function(){
    this.tabId=psclubConfig.tabId;
	this.init();	
	
	psclubPanel.superclass.constructor.call(this,{
		applyTo:this.tabId, 
	    layout:'fit',
	    ...
	    items:[{
	        ...
	        tbar: [{ 
	            ...
	            hidden:psclubConfig.auth[this.tabId+'_' + 'add']!='True',
	            handler: function(){
	                new psclubEditWindow(this.grid,true).show();            	
	            }, 
	            scope: this 
	        },'-',{ 
	        	id:this.tabId+ '_edit',
	            ...
	            hidden: psclubConfig.auth[this.tabId + '_' + 'edit'] != 'True',
	            disabled: true,
	            handler: function(){
	            	new psclubEditWindow(this.grid).show(); 
	            }, 
	            scope: this
	        },{ 
	            id:this.tabId+ '_delete',
                iconCls:'icon-remove',
                hidden: psclubConfig.auth[this.tabId + '_' + 'delete'] != 'True',
                disabled: true,
	            ...
	            iconCls:'icon-query', 
	            text:HRMSRes.Public_Toolbar_Query,
	            hidden:psclubConfig.auth[this.tabId+'_' + 'query']!='True',
	            handler: function(){
	            	new psclubQueryWindow(this.grid).show();
	            }, 
	            scope: this
	        },'-',{ 
	            ...
	            hidden:psclubConfig.auth[this.tabId+'_' + 'exportexcel']!='True',
	            handler: this.exportExcel, 
	            scope: this 
	        },'->',{ 
	        	xtype:'label', 
	            html:  '<b><font color=green>employee no: ' + psclubConfig.emno + '</font></b>',
	            hidden: psclubConfig.emno==''
	        },'->',{ 
                ...
                id:this.tabId+'_muf',
                iconCls:psclubConfig.muf=='True'?'icon-mufdelete':'icon-mufadd', 
                text:'', 
                handler: function(){
                        updateMUF(this.tabId,psclubConfig.muf?'delete':'add',psclubConfig,this.grid);
                        },
                scope: this 
            }], 
	        items:this.grid
...
Ext.extend(psclubPanel,Ext.Panel,{
    init:function(){
	...
	createGridPanel:function(){
		...
		var psclubStoreType=Ext.data.Record.create([
		    {name:'emno'},
            {name:'sfid'},
            {name:'stfn'},
            {name:'sqno'},
		...
		var store=new Ext.data.Store({ 
        	reader: new Ext.data.JsonReader({
	   	    ...
	   	    baseParams:{record:Ext.encode({params:[{ColumnName:psclubKeyColumn,ColumnValue:psclubConfig.emno}]})},   
	   		url:ContextInfo.contextPath+'/' + psclubPageName + '.mvc/list',
	   		listeners:{
        ...
        return new Ext.grid.GridPanel({
            ...
            cm: new Ext.grid.ColumnModel([ 
                ...
                {header:HRMSRes.Master_Label_clnm,sortable: true, dataIndex: 'clnm'},
                {header:HRMSRes.Personal_Label_clna,sortable: true, dataIndex: 'clna'},
                {header:HRMSRes.Personal_Label_mbty,sortable: true, dataIndex: 'mbty'},
                {header:HRMSRes.Personal_Label_jodt,sortable: true, dataIndex: 'jodt',renderer:formatDateNoTime},
	...
	remove:function(){
	    ...
	    Ext.MessageBox.show({
	        ...
	        fn: function(btn, text){
		        if (btn=='yes'){
			        Ext.Ajax.request({
	   			        url:ContextInfo.contextPath+ '/'+ psclubPageName + '.mvc/delete',
	   			        success: function(response){
	   				        var o= Ext.util.JSON.decode(response.responseText);
	   				        if(o.status=='success'){
	...
	exportExcel:function(){
	    ...
	    var params={record:Ext.encode({params:[{ColumnName:psclubKeyColumn,ColumnValue:''}],headers:header})};
	    ...
	    form.name='excelForm';
	    form.method='post';
	    form.action=ContextInfo.contextPath+ '/' + psclubPageName + '.mvc/exportexcel';
	    for(var p in params){
