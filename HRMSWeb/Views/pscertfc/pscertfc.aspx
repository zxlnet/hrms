<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="pscertfc.aspx.cs" Inherits="GotWell.HRMS.HRMSWeb.Views.pscertfc.pscertfc" %>
...
var pscertfcConfig=Ext.decode('<%=ViewData["config"] %>'); 
pscertfcConfig.auth=Ext.decode('<%=ViewData["authorization"] %>');
var pscertfcPageName = 'pscertfc';
var pscertfcKeyColumn='emno';
            
var pscertfcQueryWindow=function(grid){
	this.grid=grid;
	this.store=this.grid.getStore();
	this.init();	
	
	pscertfcQueryWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:400, 
        height:320, 
        closeAction:'close', 
...
Ext.extend(pscertfcQueryWindow,Ext.Window,{
    init:function(){
		...
		return new Ext.FormPanel({   
             ...
             items: [
                     ...
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Public_Label_sftn,
      		        ...
      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Personal_Label_cfno,
                        name: 'ctno',stateful:false,anchor:'95%'}]},
            
      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Personal_Label_cfnm,
                        name: 'ctnm',stateful:false,anchor:'95%'}]},
...
var pscertfcEditWindow=function(grid,isNew){
	this.grid=grid;
	this.isNew=isNew||false;
	this.store=this.grid.getStore();
	this.init();	

	pscertfcEditWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:650, 
        height:350, 
        ...
        listeners:{
            show:function(){              
		        var keyField = this.basisForm.findField(pscertfcKeyColumn);
		        ...
		        {
		            keyField.setValue(pscertfcConfig.emno);
		            if (pscertfcConfig.emno!='')
		                getMaxsqno(pscertfcConfig.emno,pscertfcConfig.tableName,this.basisForm);
		        }
		        
		        var keyValue = keyField.getValue();
                
                setLastModifiedInfo(pscertfcConfig,this.basisForm);
            },
            scope:this
        },
        buttons: [{ 
...
Ext.extend(pscertfcEditWindow,Ext.Window,{
    init:function(){
      		    ...
      		    items:[
                    ...
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Public_Label_staff + '(<font color=red>*</font>)',
                        ...
                        store: new Ext.data.Store({ 
                            ...
                            listeners:{select:function(p){var emno = p.getValue();if (emno!='') {getMaxsqno(emno,pscertfcConfig.tableName,this.basisForm);}},scope:this}
    		          }]},
                     ...
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Public_Label_sqno+'(<font color=red>*</font>)',
      		        ...
      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Personal_Label_cfno,
                        name: 'cfno',stateful:false,anchor:'95%'}]},

      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Personal_Label_cfnm+'(<font color=red>*</font>)',
                        name: 'cfnm',allowBlank:false,stateful:false,anchor:'95%'}]},

                    {columnWidth:.5,layout: 'form',
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Master_Label_ctnm +'(<font color=red>*</font>)',
                     ...
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Personal_Label_isnm,
                        name: 'isnm',stateful:false,anchor:'95%'}]},
                    ...
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Master_Label_cocd,
                        ...
                        store: new Ext.data.Store({ 
		                    ...
		                    listeners:{load:function(){var f = this.basisForm.findField('cocd');f.setValue(f.getValue());},scope:this}})
    		          }]},

     		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'datefield',fieldLabel:HRMSRes.Public_Label_isdt,id:'isdt',
	...
	save: function(){
		...
		Ext.Ajax.request({
		   url: ContextInfo.contextPath+'/' + pscertfcPageName + '.mvc/'+method,
...
var pscertfcPanel=function(){
    this.tabId=pscertfcConfig.tabId;
	this.init();	
	
	pscertfcPanel.superclass.constructor.call(this,{
		applyTo:this.tabId, 
	    layout:'fit',
	    ...
	    items:[{
	        ...
	        tbar: [{ 
	            ...
	            hidden:pscertfcConfig.auth[this.tabId+'_' + 'add']!='True',
	            handler: function(){
	                new pscertfcEditWindow(this.grid,true).show();            	
	            }, 
	            scope: this 
	        },'-',{ 
	        	id:this.tabId+ '_edit',
	            ...
	            hidden: pscertfcConfig.auth[this.tabId + '_' + 'edit'] != 'True',
	            disabled: true,
	            handler: function(){
	            	new pscertfcEditWindow(this.grid).show(); 
	            }, 
	            scope: this
	        },{ 
	            id:this.tabId+ '_delete',
                iconCls:'icon-remove',
                hidden: pscertfcConfig.auth[this.tabId + '_' + 'delete'] != 'True',
                disabled: true,
	            ...
	            iconCls:'icon-query', 
	            text:HRMSRes.Public_Toolbar_Query,
	            hidden:pscertfcConfig.auth[this.tabId+'_' + 'query']!='True',
	            handler: function(){
	            	new pscertfcQueryWindow(this.grid).show();
	            }, 
	            scope: this
	        },'-',{ 
	            ...
	            hidden:pscertfcConfig.auth[this.tabId+'_' + 'exportexcel']!='True',
	            handler: this.exportExcel, 
	            scope: this 
	        },'->',{ 
	        	xtype:'label', 
	            html:  '<b><font color=green>employee no: ' + pscertfcConfig.emno + '</font></b>',
	            hidden: pscertfcConfig.emno==''
	        },'->',{ 
                ...
                id:this.tabId+'_muf',
                iconCls:pscertfcConfig.muf=='True'?'icon-mufdelete':'icon-mufadd', 
                text:'', 
                handler: function(){
                        updateMUF(this.tabId,pscertfcConfig.muf?'delete':'add',pscertfcConfig,this.grid);},
                scope: this 
            }], 
	        items:this.grid
       }]
...
Ext.extend(pscertfcPanel,Ext.Panel,{
    init:function(){
		this.grid=this.createGridPanel();
	},

	createGridPanel:function(){
		var pscertfcStoreType=Ext.data.Record.create([
		    {name:'emno'},
            {name:'sfid'},
            {name:'stfn'},
            {name:'sqno'},
		...
		var store=new Ext.data.Store({ 
        	reader: new Ext.data.JsonReader({
	   	    ...
	   	    baseParams:{record:Ext.encode({params:[{ColumnName:pscertfcKeyColumn,ColumnValue:pscertfcConfig.emno}]})},   
	   		url:ContextInfo.contextPath+'/' + pscertfcPageName + '.mvc/list',
	   		listeners:{
        ...
        return new Ext.grid.GridPanel({
            ...
            cm: new Ext.grid.ColumnModel([ 
                ...
                {header:HRMSRes.Personal_Label_cfnm,sortable: true, dataIndex: 'cfnm'},
                ...
                {header:HRMSRes.Master_Label_ctnm,sortable: true, dataIndex: 'ctnm'},
                {header:HRMSRes.Personal_Label_isnm,sortable: true, dataIndex: 'isnm'},
	...
	remove:function(){
	    ...
	    Ext.MessageBox.show({
	        ...
	        fn: function(btn, text){
		        if (btn=='yes'){
			        Ext.Ajax.request({
	   			        url:ContextInfo.contextPath+ '/'+ pscertfcPageName + '.mvc/delete',
	   			        success: function(response){
	   				        var o= Ext.util.JSON.decode(response.responseText);
	   				        if(o.status=='success'){
	...
	exportExcel:function(){
	    ...
	    var params={record:Ext.encode({params:[{ColumnName:pscertfcKeyColumn,ColumnValue:''}],headers:header})};
	    ...
	    form.name='excelForm';
	    form.method='post';
	    form.action=ContextInfo.contextPath+ '/' + pscertfcPageName + '.mvc/exportexcel';
	    for(var p in params){
