<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="atshfely.aspx.cs" Inherits="GotWell.HRMS.HRMSWeb.Views.atshfely.atshfely" %>
...
var atshfelyConfig=Ext.decode('<%=ViewData["config"] %>'); 
atshfelyConfig.auth=Ext.decode('<%=ViewData["authorization"] %>');
var atshfelyPageName = 'atshfely';
var atshfelyKeyColumn='sfcd';
            
var atshfelyQueryWindow=function(grid){
	this.grid=grid;
	this.store=this.grid.getStore();
	this.init();	
	
	atshfelyQueryWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:400, 
        height:270, 
        closeAction:'close', 
...
Ext.extend(atshfelyQueryWindow,Ext.Window,{
    init:function(){
	...
	createFormPanel:function(){	        
		return new Ext.FormPanel({   
             ...
             items: [
                    ...
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Attendance_Label_sfcd ,
...
var atshfelyEditWindow=function(grid,isNew){
	this.grid=grid;
	this.isNew=isNew||false;
	this.store=this.grid.getStore();
	this.init();	
                      
	atshfelyEditWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:650, 
        height:350, 
        ...
        listeners:{
            show:function(){              
		        var keyField = this.basisForm.findField(atshfelyKeyColumn);
		        ...
		        {
		            keyField.setValue(atshfelyConfig.sfcd);
		        }
		        var keyValue = keyField.getValue();
                setLastModifiedInfo(atshfelyConfig,this.basisForm);
            },
            scope:this
        },
        buttons: [{ 
...
Ext.extend(atshfelyEditWindow,Ext.Window,{
    init:function(){
      		 ...
      		 items: [
      		    ...
      		    items:[
                    ...
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Attendance_Label_sfnm+ '(<font color=red>*</font>)' ,
      		        ...
      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'numberfield',fieldLabel:HRMSRes.Attendance_Label_eotm+ '(<font color=red>*</font>)',keepZero:false,
                        name: 'eotm',allowBlank:false,disabled:!this.isNew,stateful:false,anchor:'95%'}]},

      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'numberfield',fieldLabel:HRMSRes.Attendance_Label_eact,keepZero:false,
                        name: 'eact',allowBlank:false,stateful:false,anchor:'95%'}]},
	...
	save: function(){
        ...
        keyparams[1]={ColumnName:'eotm',ColumnValue:this.basisForm.findField('eotm').getValue(),ColumnType:'int'};
        
	    var method=this.isNew?'new':'edit';
		Ext.Ajax.request({
		   url: ContextInfo.contextPath+'/' + atshfelyPageName + '.mvc/'+method,
...
var atshfelyPanel=function(){
    this.tabId=atshfelyConfig.tabId;
	this.init();	
	
	atshfelyPanel.superclass.constructor.call(this,{
		applyTo:this.tabId, 
	    layout:'fit',
	    ...
	    items:[{
	        ...
	        tbar: [{ 
	            ...
	            hidden:atshfelyConfig.auth[this.tabId+'_' + 'add']!='True',
	            handler: function(){
	                new atshfelyEditWindow(this.grid,true).show();            	
	            }, 
	            scope: this 
	        },'-',{ 
	        	id:this.tabId+ '_edit',
	            ...
	            hidden:atshfelyConfig.auth[this.tabId+'_' + 'edit']!='True',
	            handler: function(){
	            	new atshfelyEditWindow(this.grid).show(); 
	            }, 
	            scope: this
	        },{ 
	            id:this.tabId+ '_delete',
                iconCls:'icon-remove', 
                hidden:atshfelyConfig.auth[this.tabId+'_' + 'delete']!='True',
	            ...
	            iconCls:'icon-query', 
	            text:HRMSRes.Public_Toolbar_Query,
	            hidden:atshfelyConfig.auth[this.tabId+'_' + 'query']!='True',
	            handler: function(){
	            	new atshfelyQueryWindow(this.grid).show();
	            }, 
	            scope: this
	        },'-',{ 
	            ...
	            hidden:atshfelyConfig.auth[this.tabId+'_' + 'exportexcel']!='True',
                ...
                id:this.tabId+'_muf',
                iconCls:atshfelyConfig.muf=='True'?'icon-mufdelete':'icon-mufadd', 
                text:'', 
                handler: function(){updateMUF(this.tabId,atshfelyConfig.muf?'delete':'add',atshfelyConfig,this.grid);},
                scope: this 
            }], 
	        items:this.grid
       }]
...
Ext.extend(atshfelyPanel,Ext.Panel,{
    init:function(){
	...
	createGridPanel:function(){
		...
		var atshfelyStoreType=Ext.data.Record.create([
            {name:'sfcd'},
            {name:'sfnm'},
            {name:'eotm'},
            {name:'eact'},
		...
		var store=new Ext.data.Store({ 
        	reader: new Ext.data.JsonReader({
	   	    ...
	   	    baseParams:{record:Ext.encode({params:[{ColumnName:atshfelyKeyColumn,ColumnValue:atshfelyConfig.sfcd}]})},   
	   		url:ContextInfo.contextPath+'/' + atshfelyPageName + '.mvc/list',
	   		listeners:{
        ...
        return new Ext.grid.GridPanel({
            ...
            cm: new Ext.grid.ColumnModel([ 
                ...
                {header:HRMSRes.Attendance_Label_eotm,sortable: true, dataIndex: 'eotm'},
	...
	remove:function(){
        ...
        keyparams[1]={ColumnName:'eotm',ColumnValue:record.get('eotm'),ColumnType:'int'};
	    ...
	    Ext.MessageBox.show({
	        ...
	        fn: function(btn, text){
		        if (btn=='yes'){
			        Ext.Ajax.request({
	   			        url:ContextInfo.contextPath+ '/'+ atshfelyPageName + '.mvc/delete',
	   			        success: function(response){
	   				        var o= Ext.util.JSON.decode(response.responseText);
	   				        if(o.status=='success'){
	...
	exportExcel:function(){
	    ...
	    var params={record:Ext.encode({params:[{ColumnName:atshfelyKeyColumn,ColumnValue:''}],headers:header})};
	    ...
	    form.name='excelForm';
	    form.method='post';
	    form.action=ContextInfo.contextPath+ '/' + atshfelyPageName + '.mvc/exportexcel';
	    for(var p in params){
