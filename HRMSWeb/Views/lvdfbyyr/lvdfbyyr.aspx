<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="lvdfbyyr.aspx.cs" Inherits="GotWell.HRMS.HRMSWeb.Views.lvdfbyyr.lvdfbyyr" %>
...
var lvdfbyyrConfig=Ext.decode('<%=ViewData["config"] %>'); 
lvdfbyyrConfig.auth=Ext.decode('<%=ViewData["authorization"] %>');
var lvdfbyyrPageName = 'lvdfbyyr';
var lvdfbyyrKeyColumn='ltcd';
var dtlgrdAction = 'query';
            
var lvdfbyyrQueryWindow=function(grid){
	this.grid=grid;
	this.store=this.grid.getStore();
	this.init();	
	
	lvdfbyyrQueryWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:400, 
        height:270, 
        closeAction:'close', 
...
Ext.extend(lvdfbyyrQueryWindow,Ext.Window,{
    init:function(){
...
var lvdfbyyrEditWindow=function(grid,isNew){
	this.grid=grid;
	...
	lvdfbyyrEditWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:650, 
        height:500, 
        ...
        listeners:{
            show:function(){    
		        var keyField = this.basisForm.findField(lvdfbyyrKeyColumn);
                if(!this.isNew){	
...
Ext.extend(lvdfbyyrEditWindow,Ext.Window,{
    init:function(){
                        ...
                        store: new Ext.data.Store({ 
                            ...
                            listeners:{
                                select:function(p)
                                {
                                    ...
                                    if (ltcd!='')
                                    {
                                        if (this.dtlgrid.getStore().getModifiedRecords().length>0){
	                                        Ext.MessageBox.show({
	                                            title:HRMSRes.Public_Confirm_Title,
	                                            msg:HRMSRes.Public_Message_YCWBL,
    ...
    createGridPanel:function(){
		this.lvdfbyyrDtlStoreType=Ext.data.Record.create([
            {name:'sqno'},
            {name:'fryr'},
            {name:'toyr'},
            {name:'days'},
		...
		var store=new Ext.data.Store({ 
        	reader: new Ext.data.JsonReader({
	    		...
	    		root: "rows"               
	   	 	},this.lvdfbyyrDtlStoreType), 	
	   	    baseParams:{record:Ext.encode({params:[{ColumnName:lvdfbyyrKeyColumn,ColumnValue:''}]})},   
	   		url:ContextInfo.contextPath+'/' + lvdfbyyrPageName + '.mvc/getDefDtl',
	   		listeners:{
        ...
        return new Ext.grid.EditorGridPanel({
            ...
            {header:'From year',sortable: true, dataIndex: 'fryr'},

            {id:'toyr',header: "To year",dataIndex: 'toyr',width: 100,
            ...
            {id:'days',header: "Days",dataIndex: 'days',width: 100,
                editor: new Ext.form.NumberField({allowBlank:false,allowNegative:false})},

            {id:'remk',header: "Remark",dataIndex: 'remk',width: 100,
	...
	save: function(){
        ...
        for (var i =0; i< st.getCount();i++ ){
            var x = st.getAt(i);
            var p = {ltcd:this.basisForm.findField('ltcd').getValue(),
        ...
        var keyparams=[];
        keyparams[0]={ColumnName:'ltcd',ColumnValue:this.basisForm.findField('ltcd').getValue()};
        
	    var method=this.isNew?'new':'edit';
		Ext.Ajax.request({
		   url: ContextInfo.contextPath+'/' + lvdfbyyrPageName + '.mvc/'+method,
	...
	query:function(){	    
        var params=[];	
        var f = this.basisForm.findField(lvdfbyyrKeyColumn);        
        var p={
            ColumnName:f.getName(),
            ColumnValue:f.getValue()                
        };
    ...
    addline:function(){	    
        ...
        params['fryr']= m;//==0?'0':m.toString();
        params['toyr']='';
        params['days']='0';
        params['remk']=' ';
        params['lmtm']= formatDateTime(new Date());
        params['lmur']=lvdfbyyrConfig.currentUser;

        var store = this.dtlgrid.getStore();
        store.add(new store.recordType(params));
        
        this.dtlgrid.store.getAt(this.dtlgrid.store.getCount()-1).set('remark',' ');
    },
	deleteline:function(){
	    var sm=this.dtlgrid.getSelectionModel();
	...
	exportExcelLine:function(){
	    ...
	    var params={record:Ext.encode({params:[{ColumnName:lvdfbyyrKeyColumn,ColumnValue:''}],headers:header})};
	    ...
	    form.name='excelForm';
	    form.method='post';
	    form.action=ContextInfo.contextPath+ '/' + lvdfbyyrPageName + '.mvc/exportexceldtl';
	    for(var p in params){
...
var lvdfbyyrPanel=function(){
    this.tabId=lvdfbyyrConfig.tabId;
	this.init();	
	
	lvdfbyyrPanel.superclass.constructor.call(this,{
		applyTo:this.tabId, 
	    layout:'fit',
	    ...
	    items:[{
	        ...
	        tbar: [{ 
	            ...
	            iconCls:'icon-add', 
	            text: HRMSRes.Public_Toolbar_Add, 
	            hidden:lvdfbyyrConfig.auth[this.tabId+'_' + 'add']!='True',
	            handler: function(){
	                new lvdfbyyrEditWindow(this.grid,true).show();            	
	            }, 
	            scope: this 
	        },'-',{ 
	        	id:this.tabId+ '_edit',
	            ...
	            hidden:lvdfbyyrConfig.auth[this.tabId+'_' + 'edit']!='True',
	            handler: function(){
	            	new lvdfbyyrEditWindow(this.grid).show(); 
	            }, 
	            scope: this
	        },{ 
	            id:this.tabId+ '_delete',
                iconCls:'icon-remove', 
                hidden:lvdfbyyrConfig.auth[this.tabId+'_' + 'delete']!='True',
                text:HRMSRes.Public_Toolbar_Delete, 
                handler: this.remove, 
                scope: this 
            },'-',{ 
	            ...
	            iconCls:'icon-query', 
	            text:HRMSRes.Public_Toolbar_Query,
	            hidden:lvdfbyyrConfig.auth[this.tabId+'_' + 'query']!='True',
	            handler: function(){
	            	new lvdfbyyrQueryWindow(this.grid).show();
	            }, 
	            scope: this
	        },'-',{ 
	            ...
	            iconCls:'icon-export', 
	            text:HRMSRes.Public_Toolbar_ToExcel, 
	            hidden:lvdfbyyrConfig.auth[this.tabId+'_' + 'exportexcel']!='True',
                ...
                id:this.tabId+'_muf',
                iconCls:lvdfbyyrConfig.muf=='True'?'icon-mufdelete':'icon-mufadd', 
                text:'', 
                handler: function(){updateMUF(this.tabId,lvdfbyyrConfig.muf?'delete':'add',lvdfbyyrConfig,this.grid);},
                scope: this 
            }], 
	        items:this.grid
       }]
...
Ext.extend(lvdfbyyrPanel,Ext.Panel,{
    init:function(){
	...
	createGridPanel:function(){
		...
		var lvdfbyyrStoreType=Ext.data.Record.create([
            {name:'ltcd'},
            {name:'ltnm'}
		]);
		...
		var store=new Ext.data.Store({ 
        	reader: new Ext.data.JsonReader({
	   	    ...
	   	    baseParams:{record:Ext.encode({params:[{ColumnName:lvdfbyyrKeyColumn,ColumnValue:''}]})},   
	   		url:ContextInfo.contextPath+'/' + lvdfbyyrPageName + '.mvc/list',
	   		listeners:{
                loadexception:function(o,t,response)
                {
                    var o= Ext.util.JSON.decode(response.responseText);		   		
        ...
        return new Ext.grid.GridPanel({
            ...
            cm: new Ext.grid.ColumnModel([ 
                {header:'Leave Type Name',sortable: true, dataIndex: 'ltnm',hidden:false}
            ]),
            bbar: new Ext.PagingToolbar({
                pageSize:Pagination.pagingSize,
                store: store,
	...
	remove:function(){
		...
		var keyparams=[];
        keyparams[0]={ColumnName:'ltcd',ColumnValue:record.get('ltcd')};
	    ...
	    Ext.MessageBox.show({
	        ...
	        fn: function(btn, text){
		        if (btn=='yes'){
			        Ext.Ajax.request({
	   			        url:ContextInfo.contextPath+ '/'+ lvdfbyyrPageName + '.mvc/delete',
	   			        success: function(response){
	   				        var o= Ext.util.JSON.decode(response.responseText);
	   				        if(o.status=='success'){
	...
	exportExcel:function(){
	    ...
	    form.name='excelForm';
	    form.method='post';
	    form.action=ContextInfo.contextPath+ '/' + lvdfbyyrPageName + '.mvc/exportexcel';
	    for(var p in params){
	        var hd = document.createElement('input');
            hd.type = 'hidden';
            hd.name = p;
