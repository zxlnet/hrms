<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="otlimit.aspx.cs" Inherits="GotWell.HRMS.HRMSWeb.Views.otlimit.otlimit" %>
...
var otlimitConfig=Ext.decode('<%=ViewData["config"] %>'); 
otlimitConfig.auth=Ext.decode('<%=ViewData["authorization"] %>');
var otlimitPageName = 'otlimit';
var otlimitKeyColumn='lmno';
            
var otlimitQueryWindow=function(grid){
	this.grid=grid;
	this.store=this.grid.getStore();
	this.init();	
	
	otlimitQueryWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:400, 
        height:270, 
        closeAction:'close', 
...
Ext.extend(otlimitQueryWindow,Ext.Window,{
    init:function(){
...
var otlimitEditWindow=function(grid,isNew){
	this.grid=grid;
	this.isNew=isNew||false;
	this.store=this.grid.getStore();
	this.init();	
           
	otlimitEditWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:650, 
        height:300, 
        ...
        listeners:{
            show:function(){    
		        var keyField = this.basisForm.findField(otlimitKeyColumn);
                if(!this.isNew){	
		        ...
		        {
		            getMaxNo(otlimitConfig.tableName,'lmno',this.basisForm,keyField);
		        }
                setLastModifiedInfo(otlimitConfig,this.basisForm);
        ...
        buttons: [{ 
            ...
            handler: function(){
                    ...
                    var url = ContextInfo.contextPath+'/' + otlimitPageName + '.mvc/applyto';
...
Ext.extend(otlimitEditWindow,Ext.Window,{
    init:function(){
		...
		return new Ext.FormPanel({   
	         ...
	         header:true,
      		 items: [
      		    ...
      		    items:[
       		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Overtime_Label_lmno,disabled:true,
                        name: 'lmno',stateful:false,anchor:'95%'}]},
     		        
                    {columnWidth:.5,layout: 'form',
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Overtime_Label_otcd + '(<font color=red>*</font>)',
                        name: 'otcd',stateful:false,allowBlank:false,typeAhead: true,
                        triggerAction: 'all',mode: 'local',maxHeight:150,anchor:'95%',
                        displayField: 'DisplayField',valueField:'ValueField',
                        store: new Ext.data.Store({ 
		                    reader: new Ext.data.JsonReader({totalProperty: "results",root: "rows",
		                    ...
		                    listeners:{
                                    load: function()
                                    {var v = this.basisForm.findField('otcd').getValue();this.basisForm.findField('otcd').setValue(v);},scope:this
		                        }})}]},
                                        
                    {columnWidth:.5,layout: 'form',
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Overtime_Label_lmby + '(<font color=red>*</font>)',
                        ...
                        store: new Ext.data.Store({ 
		                    ...
		                    url:ContextInfo.contextPath+'/dropdown.mvc/getOTdffrLimit',
		                    listeners:{
                                    load: function(t,r)
                                    {
                    ...
                    {columnWidth:.5,layout: 'form',
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Overtime_Label_lmva ,
                    ...
                    {columnWidth:.5,layout: 'form',
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Overtime_Label_lmsc ,
                        name: 'lmsc',stateful:false,typeAhead: true,
                        triggerAction: 'all',mode: 'local',maxHeight:150,anchor:'95%',
                        displayField: 'DisplayField',valueField:'ValueField',
                        store: LimitStore
    		          }]},
      		        ...
      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'numberfield',fieldLabel:HRMSRes.Overtime_Label_mxoh,disabled:false,
                        name: 'mxoh',allowBlank:false,stateful:false,anchor:'95%',decimalPrecision:1,keepZero:true}]},

      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'numberfield',fieldLabel:HRMSRes.Overtime_Label_mxth,disabled:false,
                        name: 'mxth',allowBlank:false,stateful:false,anchor:'95%',decimalPrecision:1,keepZero:true}]},

       		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textarea',fieldLabel:HRMSRes.Public_Label_remk,height:70,
                        name: 'remark',stateful:false,anchor:'95%'}]},
	...
	save: function(){
		...
		Ext.Ajax.request({
		   url: ContextInfo.contextPath+'/' + otlimitPageName + '.mvc/'+method,
...
var otlimitPanel=function(){
    this.tabId=otlimitConfig.tabId;
	this.init();	
	
	otlimitPanel.superclass.constructor.call(this,{
		applyTo:this.tabId, 
	    layout:'fit',
	    ...
	    items:[{
	        ...
	        tbar: [{ 
	            ...
	            hidden:otlimitConfig.auth[this.tabId+'_' + 'add']!='True',
	            handler: function(){
	                new otlimitEditWindow(this.grid,true).show();            	
	            }, 
	            scope: this 
	        },'-',{ 
	        	id:this.tabId+ '_edit',
	            ...
	            hidden: otlimitConfig.auth[this.tabId + '_' + 'edit'] != 'True',
	            disabled:true,
	            handler: function(){
	            	new otlimitEditWindow(this.grid).show(); 
	            }, 
	            scope: this
	        },{ 
	            id:this.tabId+ '_delete',
                iconCls:'icon-remove',
                hidden: otlimitConfig.auth[this.tabId + '_' + 'delete'] != 'True',
                disabled:true,
	            ...
	            iconCls:'icon-query', 
	            text:HRMSRes.Public_Toolbar_Query,
	            hidden:otlimitConfig.auth[this.tabId+'_' + 'query']!='True',
	            handler: function(){
	            	new otlimitQueryWindow(this.grid).show();
	            }, 
	            scope: this
	        },'-',{ 
	            ...
	            hidden:otlimitConfig.auth[this.tabId+'_' + 'exportexcel']!='True',
                ...
                id:this.tabId+'_muf',
                iconCls:otlimitConfig.muf=='True'?'icon-mufdelete':'icon-mufadd', 
                text:'', 
                handler: function(){updateMUF(this.tabId,otlimitConfig.muf?'delete':'add',otlimitConfig,this.grid);},
                scope: this 
            }], 
	        items:this.grid
       }]
...
Ext.extend(otlimitPanel,Ext.Panel,{
    init:function(){
	...
	createGridPanel:function(){
		...
		var otlimitStoreType=Ext.data.Record.create([
            {name:'lmno'},
            {name:'otcd'},
            {name:'otnm'},
            {name:'lmby'},
		...
		var store=new Ext.data.Store({ 
        	reader: new Ext.data.JsonReader({
	   	    ...
	   	    baseParams:{record:Ext.encode({params:[{ColumnName:otlimitKeyColumn,ColumnValue:''}]})},   
	   		url:ContextInfo.contextPath+'/' + otlimitPageName + '.mvc/list',
	   		listeners:{
        ...
        return new Ext.grid.GridPanel({
            ...
            cm: new Ext.grid.ColumnModel([ 
                {header:HRMSRes.Overtime_Label_lmno,sortable: true, dataIndex: 'lmno'},
                {header:HRMSRes.Overtime_Label_otnm,sortable: true, dataIndex: 'otnm'},
                {header:'lmbytext',sortable: true, dataIndex: 'lmbytext'},
                {header:HRMSRes.Overtime_Label_lmtx,sortable: true, dataIndex: 'lmtx'},
                {header:HRMSRes.Overtime_Label_lmsc,sortable: true, dataIndex: 'lmsc'},
                {header:HRMSRes.Overtime_Label_mxoh,sortable: true, dataIndex: 'mxoh'},
                {header:HRMSRes.Overtime_Label_mxth,sortable: true, dataIndex: 'mxth'},
	...
	remove:function(){
	    ...
	    Ext.MessageBox.show({
	        ...
	        fn: function(btn, text){
		        if (btn=='yes'){
			        Ext.Ajax.request({
	   			        url:ContextInfo.contextPath+ '/'+ otlimitPageName + '.mvc/delete',
	   			        success: function(response){
	   				        var o= Ext.util.JSON.decode(response.responseText);
	   				        if(o.status=='success'){
	...
	exportExcel:function(){
	    ...
	    var params={record:Ext.encode({params:[{ColumnName:otlimitKeyColumn,ColumnValue:''}],headers:header})};
	    ...
	    form.name='excelForm';
	    form.method='post';
	    form.action=ContextInfo.contextPath+ '/' + otlimitPageName + '.mvc/exportexcel';
	    for(var p in params){
