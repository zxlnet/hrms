<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="atcldwkds.aspx.cs" Inherits="GotWell.HRMS.HRMSWeb.Views.atcldwkds.atcldwkds" %>
...
var atcldwkdsConfig=Ext.decode('<%=ViewData["config"] %>'); 
atcldwkdsConfig.auth=Ext.decode('<%=ViewData["authorization"] %>');
var atcldwkdsPageName = 'atcldwkds';
var atcldwkdsKeyColumn='clcd';
            
var atcldwkdsQueryWindow=function(grid){
	this.grid=grid;
	this.store=this.grid.getStore();
	this.init();	
	
	atcldwkdsQueryWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:400, 
        height:270, 
        closeAction:'close', 
...
Ext.extend(atcldwkdsQueryWindow,Ext.Window,{
    init:function(){
	...
	createFormPanel:function(){	        
		return new Ext.FormPanel({   
             ...
             items: [
                    ...
                    items: [{xtype:'combo',fieldLabel:'clcd' ,
...
var atcldwkdsEditWindow=function(grid,isNew){
	this.grid=grid;
    ...
    this.personalTab=new Ext.TabPanel({
                ...
                items:[{
                            id:'tab1',
                            title: 'Calendar wkdas Setting',
                            items: this.basisFormPanel
                        }]
            }),
                       
	atcldwkdsEditWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:650, 
        height:350, 
        ...
        listeners:{
            show:function(){              
		        var keyField = this.basisForm.findField(atcldwkdsKeyColumn);
                ...
                setLastModifiedInfo(atcldwkdsConfig,this.basisForm);
            },
            scope:this
        },
        buttons: [{ 
...
Ext.extend(atcldwkdsEditWindow,Ext.Window,{
    init:function(){
		...
		return new Ext.FormPanel({   
	         ...
	         header:true,
      		 items: [
      		    ...
      		    items:[
                    {columnWidth:.5,layout: 'form',
                    items: [{xtype:'combo',fieldLabel:'clcd'+ '(<font color=red>*</font>)' ,
                        name: 'clcd',stateful:false,typeAhead: true,allowBlank:false,
                        triggerAction: 'all',mode: 'local',maxHeight:150,anchor:'95%',
                        displayField: 'DisplayField',valueField:'ValueField',
                        store: new Ext.data.Store({ 
		                    reader: new Ext.data.JsonReader({totalProperty: "results",root: "rows",
                    ...
                    {columnWidth:.5,layout: 'form',
                    items: [{xtype:'combo',fieldLabel:'period'+ '(<font color=red>*</font>)' ,
                        name: 'period',stateful:false,typeAhead: true,allowBlank:false,
                        triggerAction: 'all',mode: 'local',maxHeight:150,anchor:'95%',
                        displayField: 'DisplayField',valueField:'ValueField',
                        store: new Ext.data.Store({ 
		                    reader: new Ext.data.JsonReader({totalProperty: "results",root: "rows",
      		        ...
      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:'wkdas'+ '(<font color=red>*</font>)',
                        name: 'wkdas',allowBlank:false,stateful:false,anchor:'95%'}]},
	...
	save: function(){
        ...
        keyparams[1]={ColumnName:'period',ColumnValue:this.basisForm.findField('period').getValue(),ColumnType:'string'};
        
	    var method=this.isNew?'new':'edit';
		Ext.Ajax.request({
		   url: ContextInfo.contextPath+'/' + atcldwkdsPageName + '.mvc/'+method,
...
var atcldwkdsPanel=function(){
    this.tabId=atcldwkdsConfig.tabId;
	this.init();	
	
	atcldwkdsPanel.superclass.constructor.call(this,{
		applyTo:this.tabId, 
	    layout:'fit',
	    ...
	    items:[{
	        ...
	        tbar: [{ 
	            ...
	            hidden:atcldwkdsConfig.auth[this.tabId+'_' + 'add']!='True',
	            handler: function(){
	                new atcldwkdsEditWindow(this.grid,true).show();            	
	            }, 
	            scope: this 
	        },'-',{ 
	        	id:this.tabId+ '_edit',
	            ...
	            hidden:atcldwkdsConfig.auth[this.tabId+'_' + 'edit']!='True',
	            handler: function(){
	            	new atcldwkdsEditWindow(this.grid).show(); 
	            }, 
	            scope: this
	        },{ 
	            id:this.tabId+ '_delete',
                iconCls:'icon-remove', 
                hidden:atcldwkdsConfig.auth[this.tabId+'_' + 'delete']!='True',
	            ...
	            iconCls:'icon-query', 
	            text:HRMSRes.Public_Toolbar_Query,
	            hidden:atcldwkdsConfig.auth[this.tabId+'_' + 'query']!='True',
	            handler: function(){
	            	new atcldwkdsQueryWindow(this.grid).show();
	            }, 
	            scope: this
	        },'-',{ 
	            ...
	            hidden:atcldwkdsConfig.auth[this.tabId+'_' + 'exportexcel']!='True',
                ...
                id:this.tabId+'_muf',
                iconCls:atcldwkdsConfig.muf=='True'?'icon-mufdelete':'icon-mufadd', 
                text:'', 
                handler: function(){updateMUF(this.tabId,atcldwkdsConfig.muf?'delete':'add',atcldwkdsConfig,this.grid);},
                scope: this 
            }], 
	        items:this.grid
       }]
...
Ext.extend(atcldwkdsPanel,Ext.Panel,{
    init:function(){
	...
	createGridPanel:function(){
		...
		var atcldwkdsStoreType=Ext.data.Record.create([
            {name:'clcd'},
            {name:'cdnm'},
            {name:'period'},
            {name:'wkdas'},
		...
		var store=new Ext.data.Store({ 
        	reader: new Ext.data.JsonReader({
	   	    ...
	   	    baseParams:{record:Ext.encode({params:[{ColumnName:atcldwkdsKeyColumn,ColumnValue:''}]})},   
	   		url:ContextInfo.contextPath+'/' + atcldwkdsPageName + '.mvc/list',
	   		listeners:{
        ...
        return new Ext.grid.GridPanel({
            ...
            cm: new Ext.grid.ColumnModel([ 
                {header:'clcd',sortable: true, dataIndex: 'clcd',hidden:false},
                {header:'cdnm',sortable: true, dataIndex: 'cdnm',hidden:false},
                {header:'period',sortable: true, dataIndex: 'period'},
                {header:'wkdas',sortable: true, dataIndex: 'wkdas'},
	...
	remove:function(){
        ...
        keyparams[1]={ColumnName:'period',ColumnValue:formatDateNoTime(record.get('period')),ColumnType:'datetime'};
	    ...
	    Ext.MessageBox.show({
	        ...
	        fn: function(btn, text){
		        if (btn=='yes'){
			        Ext.Ajax.request({
	   			        url:ContextInfo.contextPath+ '/'+ atcldwkdsPageName + '.mvc/delete',
	   			        success: function(response){
	   				        var o= Ext.util.JSON.decode(response.responseText);
	   				        if(o.status=='success'){
	...
	exportExcel:function(){
	    ...
	    var params={record:Ext.encode({params:[{ColumnName:atcldwkdsKeyColumn,ColumnValue:''}],headers:header})};
	    ...
	    form.name='excelForm';
	    form.method='post';
	    form.action=ContextInfo.contextPath+ '/' + atcldwkdsPageName + '.mvc/exportexcel';
	    for(var p in params){
