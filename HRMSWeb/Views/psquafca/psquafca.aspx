<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="psquafca.aspx.cs" Inherits="GotWell.HRMS.HRMSWeb.Views.psquafca.psquafca" %>
...
var psquafcaConfig=Ext.decode('<%=ViewData["config"] %>'); 
psquafcaConfig.auth=Ext.decode('<%=ViewData["authorization"] %>');
var psquafcaPageName = 'psquafca';
var psquafcaKeyColumn='emno';
            
var psquafcaQueryWindow=function(grid){
	this.grid=grid;
	this.store=this.grid.getStore();
	this.init();	
	
	psquafcaQueryWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:400, 
        height:320, 
        closeAction:'close', 
...
Ext.extend(psquafcaQueryWindow,Ext.Window,{
    init:function(){
		...
		return new Ext.FormPanel({   
             ...
             items: [
                     ...
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Public_Label_sftn,
      		        ...
      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Personal_Label_qunm,
                        name: 'qunm',stateful:false,anchor:'95%'}]},
            
                    {columnWidth:.5,layout: 'form',
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Master_Label_qfnm,
                        name: 'qfcd',stateful:false,typeAhead: true,
                        ...
                        store: new Ext.data.Store({ 
		                    ...
		                    url:ContextInfo.contextPath+'/dropdown.mvc/getQualType',
		                    listeners:{load:function(){f = this.form.findField('qfcd');f.setValue(f.getValue());},scope:this}})
    		          }]},

      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Personal_Label_isnm,
...
var psquafcaEditWindow=function(grid,isNew){
	this.grid=grid;
	this.isNew=isNew||false;
	this.store=this.grid.getStore();
	this.init();	
                      
	psquafcaEditWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:650, 
        height:360, 
        ...
        listeners:{
            show:function(){              
		        var keyField = this.basisForm.findField(psquafcaKeyColumn);
		        ...
		        {
		            keyField.setValue(psquafcaConfig.emno);
		            if (psquafcaConfig.emno!='')
		                getMaxsqno(psquafcaConfig.emno,psquafcaConfig.tableName,this.basisForm);
		        }
		        var keyValue = keyField.getValue();
                setLastModifiedInfo(psquafcaConfig,this.basisForm);
            },
            scope:this
        },
        buttons: [{ 
...
Ext.extend(psquafcaEditWindow,Ext.Window,{
    init:function(){
      		    ...
      		    items:[
                    ...
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Public_Label_staff + '(<font color=red>*</font>)',
                        ...
                        store: new Ext.data.Store({ 
                            ...
                            listeners:{select:function(p){var emno = p.getValue();if (emno!=''){getMaxsqno(emno,psquafcaConfig.tableName,this.basisForm);}},scope:this}
    		          }]},
                    ...
                    {columnWidth:.5,layout: 'form',
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Master_Label_qfnm +'(<font color=red>*</font>)',
                        name: 'qfcd',stateful:false,typeAhead: true,allowBlank:false,
                        triggerAction: 'all',mode: 'local',maxHeight:150,anchor:'95%',
                        displayField: 'DisplayField',valueField:'ValueField',
                        store: new Ext.data.Store({ 
		                    reader: new Ext.data.JsonReader({totalProperty: "results",root: "rows",
			                    fields: ['ValueField','DisplayField']}), autoLoad:true,
		                    url:ContextInfo.contextPath+'/dropdown.mvc/getQualType',
		                    listeners:{load:function(){f = this.basisForm.findField('qfcd');f.setValue(f.getValue());},scope:this}})
    		          }]},

      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Personal_Label_qucd+'(<font color=red>*</font>)',
                        name: 'qucd',allowBlank:false,stateful:false,anchor:'95%'}]},

      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Personal_Label_qunm+'(<font color=red>*</font>)',
                        name: 'qunm',allowBlank:false,stateful:false,anchor:'95%'}]},
                    ...
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Master_Label_conm ,
     		        ...
     		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'datefield',fieldLabel:HRMSRes.Personal_Label_obyr,id:'obyr',
                        name:'obyr',editable:false,height:22,anchor:'95%',
                     ...
                     items: [{xtype:'datefield',fieldLabel:HRMSRes.Personal_Label_frdt,id:'frdt',
                        name:'frdt',editable:false,height:22,anchor:'95%',
                        format: DATE_FORMAT.DATEONLY,minValue: '1980/01/01',stateful:false,disabled:false,
           	 	        invalidText : '{0} '+ HRMSRes.Public_Messsage_FormatInCorrect +' {1}'}]},

     		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'datefield',fieldLabel:HRMSRes.Personal_Label_todt,id:'todt',
                        name:'todt',editable:false,height:22,anchor:'95%',
                        format: DATE_FORMAT.DATEONLY,minValue: '1980/01/01',stateful:false,disabled:false,
           	 	        invalidText : '{0} '+ HRMSRes.Public_Messsage_FormatInCorrect +' {1}'}]},
     		        ...
     		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Personal_Label_stge,
                        name: 'stge',stateful:false,anchor:'95%'}]},
	...
	save: function(){
		...
		Ext.Ajax.request({
		   url: ContextInfo.contextPath+'/' + psquafcaPageName + '.mvc/'+method,
...
var psquafcaPanel=function(){
    this.tabId=psquafcaConfig.tabId;
	this.init();	
	
	psquafcaPanel.superclass.constructor.call(this,{
		applyTo:this.tabId, 
	    layout:'fit',
	    ...
	    items:[{
	        ...
	        tbar: [{ 
	            ...
	            hidden:psquafcaConfig.auth[this.tabId+'_' + 'add']!='True',
	            handler: function(){
	                new psquafcaEditWindow(this.grid,true).show();            	
	            }, 
	            scope: this 
	        },'-',{ 
	        	id:this.tabId+ '_edit',
	            ...
	            hidden: psquafcaConfig.auth[this.tabId + '_' + 'edit'] != 'True',
	            disabled: true,
	            handler: function(){
	            	new psquafcaEditWindow(this.grid).show(); 
	            }, 
	            scope: this
	        },{ 
	            id:this.tabId+ '_delete',
                iconCls:'icon-remove',
                hidden: psquafcaConfig.auth[this.tabId + '_' + 'delete'] != 'True',
                disabled: true,
	            ...
	            iconCls:'icon-query', 
	            text:HRMSRes.Public_Toolbar_Query,
	            hidden:psquafcaConfig.auth[this.tabId+'_' + 'query']!='True',
	            handler: function(){
	            	new psquafcaQueryWindow(this.grid).show();
	            }, 
	            scope: this
	        },'-',{ 
	            ...
	            hidden:psquafcaConfig.auth[this.tabId+'_' + 'exportexcel']!='True',
	            handler: this.exportExcel, 
	            scope: this 
	        },'->',{ 
	        	xtype:'label', 
	            html:  '<b><font color=green>employee no: ' + psquafcaConfig.emno + '</font></b>',
	            hidden: psquafcaConfig.emno==''
	        },'->',{ 
                ...
                id:this.tabId+'_muf',
                iconCls:psquafcaConfig.muf=='True'?'icon-mufdelete':'icon-mufadd', 
                text:'', 
                handler: function(){updateMUF(this.tabId,psquafcaConfig.muf?'delete':'add',psquafcaConfig,this.grid);},
                scope: this 
            }], 
	        items:this.grid
       }]
...
Ext.extend(psquafcaPanel,Ext.Panel,{
    init:function(){
		this.grid=this.createGridPanel();
	},

	createGridPanel:function(){
		var psquafcaStoreType=Ext.data.Record.create([
		    {name:'emno'},
            {name:'sfid'},
            {name:'stfn'},
            {name:'sqno'},
		...
		var store=new Ext.data.Store({ 
        	reader: new Ext.data.JsonReader({
	   	    ...
	   	    baseParams:{record:Ext.encode({params:[{ColumnName:psquafcaKeyColumn,ColumnValue:psquafcaConfig.emno}]})},   
	   		url:ContextInfo.contextPath+'/' + psquafcaPageName + '.mvc/list',
	   		listeners:{
        ...
        return new Ext.grid.GridPanel({
            ...
            cm: new Ext.grid.ColumnModel([ 
                ...
                {header:HRMSRes.Personal_Label_qunm,sortable: true, dataIndex: 'qunm'},
                {header:HRMSRes.Master_Label_qfnm,sortable: true, dataIndex: 'qfnnm'},
                {header:HRMSRes.Personal_Label_isnm,sortable: true, dataIndex: 'innm'},
                {header:HRMSRes.Personal_Label_obyr,sortable: true, dataIndex: 'obyr',renderer:formatDateNoTime},
	...
	remove:function(){
	    ...
	    Ext.MessageBox.show({
	        ...
	        fn: function(btn, text){
		        if (btn=='yes'){
			        Ext.Ajax.request({
	   			        url:ContextInfo.contextPath+ '/'+ psquafcaPageName + '.mvc/delete',
	   			        success: function(response){
	   				        var o= Ext.util.JSON.decode(response.responseText);
	   				        if(o.status=='success'){
	...
	exportExcel:function(){
	    ...
	    var params={record:Ext.encode({params:[{ColumnName:psquafcaKeyColumn,ColumnValue:''}],headers:header})};
	    ...
	    form.name='excelForm';
	    form.method='post';
	    form.action=ContextInfo.contextPath+ '/' + psquafcaPageName + '.mvc/exportexcel';
	    for(var p in params){
