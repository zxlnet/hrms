<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="atpvtcld.aspx.cs" Inherits="GotWell.HRMS.HRMSWeb.Views.atpvtcld.atpvtcld" %>
...
var atpvtcldConfig=Ext.decode('<%=ViewData["config"] %>'); 
atpvtcldConfig.auth=Ext.decode('<%=ViewData["authorization"] %>');
var atpvtcldPageName = 'atpvtcld';
var atpvtcldKeyColumn='emno';
            
var atpvtcldQueryWindow=function(grid){
	this.grid=grid;
	this.store=this.grid.getStore();
	this.init();	
	
	atpvtcldQueryWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:400, 
        height:270, 
        closeAction:'close', 
...
Ext.extend(atpvtcldQueryWindow,Ext.Window,{
    init:function(){
		...
		return new Ext.FormPanel({   
             ...
             items: [
                     ...
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Public_Label_sftn,
      		        ...
      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'datefield',fieldLabel:HRMSRes.Attendance_Label_cddt + '' + HRMSRes.Public_Label_From,id:'from|cddt',
                        name:'from|cddt',editable:false,height:22,anchor:'95%',
                        format: DATE_FORMAT.DATEONLY,minValue: '1980/01/01',
           	 	        invalidText : '{0} '+ HRMSRes.Public_Messsage_FormatInCorrect +' {1}'}]},

     		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'datefield',fieldLabel:HRMSRes.Attendance_Label_cddt + '' + HRMSRes.Public_Label_To,id:'to|cddt',
                        name:'to|cddt',editable:false,height:22,anchor:'95%',
                        format: DATE_FORMAT.DATEONLY,minValue: '1980/01/01',
           	 	        invalidText : '{0} '+ HRMSRes.Public_Messsage_FormatInCorrect +' {1}'}]},
...
var atpvtcldEditWindow=function(grid,isNew){
	this.grid=grid;
	this.isNew=isNew||false;
	this.store=this.grid.getStore();
	this.init();	

	atpvtcldEditWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:650, 
        height:280, 
        ...
        listeners:{
            show:function(){              
		        var keyField = this.basisForm.findField(atpvtcldKeyColumn);
                ...
                setLastModifiedInfo(atpvtcldConfig,this.basisForm);
            },
            scope:this
        },
        buttons: [{ 
...
Ext.extend(atpvtcldEditWindow,Ext.Window,{
    init:function(){
      		 ...
      		 items: [
      		    ...
      		    items:[
                    ...
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Public_Label_staff + '(<font color=red>*</font>)',
     		        ...
     		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'datefield',fieldLabel:HRMSRes.Attendance_Label_cddt,id:'cddt',
                        name:'cddt',editable:false,height:22,anchor:'95%',
                        format: DATE_FORMAT.DATE_ONLY_WITH_SPILITOR_1,minValue: '1980/01/01',stateful:false,disabled:false,
           	 	        invalidText : '{0} '+ HRMSRes.Public_Messsage_FormatInCorrect +' {1}'}]},

                    {columnWidth:.5,layout: 'form',
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Attendance_Label_htcd ,
                        ...
                        store: new Ext.data.Store({ 
		                    ...
		                    listeners:{
                                load:function(){f = this.basisForm.findField('htcd');f.setValue(f.getValue());},
                                scope:this}})		                    
    		          }]},

                    {columnWidth:.5,layout: 'form',
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Attendance_Label_otcd ,
                        ...
                        store: new Ext.data.Store({ 
		                    ...
		                    listeners:{
                                load:function(){f = this.basisForm.findField('otcd');f.setValue(f.getValue());},
                                scope:this}})		                    
    		          }]},
	...
	save: function(){
        ...
        keyparams[1]={ColumnName:'cddt',ColumnValue:formatDateNoTime(this.basisForm.findField('cddt').getValue()),ColumnType:'datetime'};
        
	    var method=this.isNew?'new':'edit';
		Ext.Ajax.request({
		   url: ContextInfo.contextPath+'/' + atpvtcldPageName + '.mvc/'+method,
	...
	applyto: function(){
        ...
        var url = ContextInfo.contextPath+'/' + atpvtcldPageName + '.mvc/applyto';
        new empAdvQryQueryWindow(this.grid,'1',url,params).show();
	}
});


var atpvtcldPanel=function(){
    this.tabId=atpvtcldConfig.tabId;
	this.init();	
	
	atpvtcldPanel.superclass.constructor.call(this,{
		applyTo:this.tabId, 
	    layout:'fit',
	    ...
	    items:[{
	        ...
	        tbar: [{ 
	            ...
	            hidden:atpvtcldConfig.auth[this.tabId+'_' + 'add']!='True',
	            handler: function(){
	                new atpvtcldEditWindow(this.grid,true).show();            	
	            }, 
	            scope: this 
	        },'-',{ 
	        	id:this.tabId+ '_edit',
	            ...
	            hidden:atpvtcldConfig.auth[this.tabId+'_' + 'edit']!='True',
	            handler: function(){
	            	new atpvtcldEditWindow(this.grid).show(); 
	            }, 
	            scope: this
	        },{ 
	            id:this.tabId+ '_delete',
                iconCls:'icon-remove', 
                hidden:atpvtcldConfig.auth[this.tabId+'_' + 'delete']!='True',
	            ...
	            iconCls:'icon-query', 
	            text:HRMSRes.Public_Toolbar_Query,
	            hidden:atpvtcldConfig.auth[this.tabId+'_' + 'query']!='True',
	            handler: function(){
	            	new atpvtcldQueryWindow(this.grid).show();
	            }, 
	            scope: this
	        },'-',{ 
	            ...
	            hidden:atpvtcldConfig.auth[this.tabId+'_' + 'exportexcel']!='True',
                ...
                id:this.tabId+'_muf',
                iconCls:atpvtcldConfig.muf=='True'?'icon-mufdelete':'icon-mufadd', 
                text:'', 
                handler: function(){updateMUF(this.tabId,atpvtcldConfig.muf?'delete':'add',atpvtcldConfig,this.grid);},
                scope: this 
            }], 
	        items:this.grid
       }]
...
Ext.extend(atpvtcldPanel,Ext.Panel,{
    init:function(){
	...
	createGridPanel:function(){
		...
		var atpvtcldStoreType=Ext.data.Record.create([
		    {name:'emno'},
            {name:'sfid'},
            {name:'stfn'},
            {name:'cddt'},
		...
		var store=new Ext.data.Store({ 
        	reader: new Ext.data.JsonReader({
	   	    ...
	   	    baseParams:{record:Ext.encode({params:[{ColumnName:atpvtcldKeyColumn,ColumnValue:atpvtcldConfig.emno}]})},   
	   		url:ContextInfo.contextPath+'/' + atpvtcldPageName + '.mvc/list',
	   		listeners:{
        ...
        return new Ext.grid.GridPanel({
            ...
            cm: new Ext.grid.ColumnModel([ 
                ...
                {header:HRMSRes.Attendance_Label_cddt,sortable: true, dataIndex: 'cddt',renderer:formatDateNoTime},
                {header:HRMSRes.Attendance_Label_htnm ,sortable: true, dataIndex: 'htnm'},
                {header:HRMSRes.Attendance_Label_otnm ,sortable: true, dataIndex: 'otnm'},
	...
	remove:function(){
        ...
        keyparams[1]={ColumnName:'cddt',ColumnValue:formatDateNoTime(record.get('cddt')),ColumnType:'datetime'};
	    ...
	    Ext.MessageBox.show({
	        ...
	        fn: function(btn, text){
		        if (btn=='yes'){
			        Ext.Ajax.request({
	   			        url:ContextInfo.contextPath+ '/'+ atpvtcldPageName + '.mvc/delete',
	   			        success: function(response){
	   				        var o= Ext.util.JSON.decode(response.responseText);
	   				        if(o.status=='success'){
	...
	exportExcel:function(){
	    ...
	    var params={record:Ext.encode({params:[{ColumnName:atpvtcldKeyColumn,ColumnValue:''}],headers:header})};
	    ...
	    form.name='excelForm';
	    form.method='post';
	    form.action=ContextInfo.contextPath+ '/' + atpvtcldPageName + '.mvc/exportexcel';
	    for(var p in params){
