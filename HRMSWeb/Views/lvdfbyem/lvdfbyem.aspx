<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="lvdfbyem.aspx.cs" Inherits="GotWell.HRMS.HRMSWeb.Views.lvdfbyem.lvdfbyem" %>
...
var lvdfbyempConfig=Ext.decode('<%=ViewData["config"] %>'); 
lvdfbyempConfig.auth=Ext.decode('<%=ViewData["authorization"] %>');
var lvdfbyempPageName = 'lvdfbyem';
var lvdfbyempKeyColumn='emno';
            
var lvdfbyempQueryWindow=function(grid){
	this.grid=grid;
	this.store=this.grid.getStore();
	this.init();	
	
	lvdfbyempQueryWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:400, 
        height:300, 
        closeAction:'close', 
...
Ext.extend(lvdfbyempQueryWindow,Ext.Window,{
    init:function(){
...
var lvdfbyempEditWindow=function(grid,isNew){
	this.grid=grid;
	this.isNew=isNew||false;
	this.store=this.grid.getStore();
	this.init();	
                     
	lvdfbyempEditWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:650, 
        height:350, 
        ...
        listeners:{
            show:function(){              
		        var keyField = this.basisForm.findField(lvdfbyempKeyColumn);
		        ...
		        {
		            keyField.setValue(lvdfbyempConfig.emno);
		            if (lvdfbyempConfig.emno!='')
		                getMaxsqno(lvdfbyempConfig.emno,lvdfbyempConfig.tableName,this.basisForm);
		        }
		        var keyValue = keyField.getValue();
                setLastModifiedInfo(lvdfbyempConfig,this.basisForm);
            },
            scope:this
        },
        buttons: [{ 
            ...
            handler: function(){
                    ...
                    var url = ContextInfo.contextPath+'/' + lvdfbyempPageName + '.mvc/applyto';
...
Ext.extend(lvdfbyempEditWindow,Ext.Window,{
    init:function(){
      		    ...
      		    items:[
                    ...
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Public_Label_staff + '(<font color=red>*</font>)',
                        ...
                        store: new Ext.data.Store({ 
                            ...
                            listeners:{select:function(p) {var emno = p.getValue();if (emno!=''){getMaxsqno(emno,lvdfbyempConfig.tableName,this.basisForm);}},scope:this}
    		          }]},
                    ...
                    {columnWidth:.5,layout: 'form',
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Leave_Label_ltnm ,
                        name: 'ltcd',stateful:false,typeAhead: true,
                        triggerAction: 'all',mode: 'local',maxHeight:150,anchor:'95%',
                        displayField: 'DisplayField',valueField:'ValueField',
                        store: new Ext.data.Store({ 
                     ...
                     items: [{xtype:'numberfield',fieldLabel:HRMSRes.Leave_Label_days,disabled:false,
                        name: 'days',allowBlank:false,stateful:false,anchor:'95%',decimalPrecision:1,keepZero:true}]},

     		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'datefield',fieldLabel:HRMSRes.Public_Label_exdt+'(<font color=red>*</font>)',id:'exdt',
      		        ...
      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textarea',fieldLabel:HRMSRes.Public_Label_remk,
	...
	save: function(){
        ...
        keyparams[1]={ColumnName:'sqno',ColumnValue:this.basisForm.findField('sqno').getValue(),ColumnType:'int'};
        
	    var method=this.isNew?'new':'edit';
		Ext.Ajax.request({
		   url: ContextInfo.contextPath+'/' + lvdfbyempPageName + '.mvc/'+method,
...
var lvdfbyempPanel=function(){
    this.tabId=lvdfbyempConfig.tabId;
	this.init();	
	
	lvdfbyempPanel.superclass.constructor.call(this,{
		applyTo:this.tabId, 
	    layout:'fit',
	    ...
	    items:[{
	        ...
	        tbar: [{ 
	            ...
	            hidden:lvdfbyempConfig.auth[this.tabId+'_' + 'add']!='True',
	            handler: function(){
	                new lvdfbyempEditWindow(this.grid,true).show();            	
	            }, 
	            scope: this 
	        },'-',{ 
	        	id:this.tabId+ '_edit',
	            ...
	            hidden:lvdfbyempConfig.auth[this.tabId+'_' + 'edit']!='True',
	            handler: function(){
	            	new lvdfbyempEditWindow(this.grid).show(); 
	            }, 
	            scope: this
	        },{ 
	            id:this.tabId+ '_delete',
                iconCls:'icon-remove', 
                hidden:lvdfbyempConfig.auth[this.tabId+'_' + 'delete']!='True',
	            ...
	            iconCls:'icon-query', 
	            text:HRMSRes.Public_Toolbar_Query,
	            hidden:lvdfbyempConfig.auth[this.tabId+'_' + 'query']!='True',
	            handler: function(){
	            	new lvdfbyempQueryWindow(this.grid).show();
	            }, 
	            scope: this
	        },'-',{ 
	            ...
	            hidden:lvdfbyempConfig.auth[this.tabId+'_' + 'exportexcel']!='True',
	            handler: this.exportExcel, 
	            scope: this 
	        },'->',{ 
	        	xtype:'label', 
	            html:  '<b><font color=green>employee no: ' + lvdfbyempConfig.emno + '</font></b>',
	            hidden: lvdfbyempConfig.emno==''
	        },'->',{ 
                ...
                id:this.tabId+'_muf',
                iconCls:lvdfbyempConfig.muf=='True'?'icon-mufdelete':'icon-mufadd', 
                text:'', 
                handler: function(){updateMUF(this.tabId,lvdfbyempConfig.muf?'delete':'add',lvdfbyempConfig,this.grid);},
                scope: this 
            }], 
	        items:this.grid
       }]
...
Ext.extend(lvdfbyempPanel,Ext.Panel,{
    init:function(){
	...
	createGridPanel:function(){
		...
		var lvdfbyempStoreType=Ext.data.Record.create([
		    {name:'emno'},
            {name:'sfid'},
            {name:'stfn'},
            {name:'sqno'},
		...
		var store=new Ext.data.Store({ 
        	reader: new Ext.data.JsonReader({
	   	    ...
	   	    baseParams:{record:Ext.encode({params:[{ColumnName:lvdfbyempKeyColumn,ColumnValue:lvdfbyempConfig.emno}]})},   
	   		url:ContextInfo.contextPath+'/' + lvdfbyempPageName + '.mvc/list',
	   		listeners:{
	...
	remove:function(){
        ...
        keyparams[1]={ColumnName:'sqno',ColumnValue:record.get('sqno'),ColumnType:'int'};
	    ...
	    Ext.MessageBox.show({
	        ...
	        fn: function(btn, text){
		        if (btn=='yes'){
			        Ext.Ajax.request({
	   			        url:ContextInfo.contextPath+ '/'+ lvdfbyempPageName + '.mvc/delete',
	   			        success: function(response){
	   				        var o= Ext.util.JSON.decode(response.responseText);
	   				        if(o.status=='success'){
	...
	exportExcel:function(){
	    ...
	    var params={record:Ext.encode({params:[{ColumnName:lvdfbyempKeyColumn,ColumnValue:''}],headers:header})};
	    ...
	    form.name='excelForm';
	    form.method='post';
	    form.action=ContextInfo.contextPath+ '/' + lvdfbyempPageName + '.mvc/exportexcel';
	    for(var p in params){
