<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="atshflat.aspx.cs" Inherits="GotWell.HRMS.HRMSWeb.Views.atshflat.atshflat" %>
...
var atshflatConfig=Ext.decode('<%=ViewData["config"] %>'); 
atshflatConfig.auth=Ext.decode('<%=ViewData["authorization"] %>');
var atshflatPageName = 'atshflat';
var atshflatKeyColumn='sfcd';
            
var atshflatQueryWindow=function(grid){
	this.grid=grid;
	this.store=this.grid.getStore();
	this.init();	
	
	atshflatQueryWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:400, 
        height:270, 
        closeAction:'close', 
...
Ext.extend(atshflatQueryWindow,Ext.Window,{
    init:function(){
...
var atshflatEditWindow=function(grid,isNew){
	this.grid=grid;
	this.isNew=isNew||false;
	this.store=this.grid.getStore();
	this.init();	

	atshflatEditWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:650, 
        height:350, 
        ...
        listeners:{
            show:function(){              
		        var keyField = this.basisForm.findField(atshflatKeyColumn);
		        ...
		        {
		            keyField.setValue(atshflatConfig.sfcd);
		        }
		        var keyValue = keyField.getValue();
                setLastModifiedInfo(atshflatConfig,this.basisForm);
            },
            scope:this
        },
        buttons: [{ 
...
Ext.extend(atshflatEditWindow,Ext.Window,{
    init:function(){
      		 ...
      		 items: [
      		    ...
      		    items:[
                    ...
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Attendance_Label_sfnm+ '(<font color=red>*</font>)' ,
      		        ...
      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'numberfield',fieldLabel:HRMSRes.Attendance_Label_letm+ '(<font color=red>*</font>)',
                        name: 'letm',allowBlank:false,disabled:!this.isNew,stateful:false,anchor:'95%',keepZero:false}]},

      		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'numberfield',fieldLabel:HRMSRes.Attendance_Label_lact,
                        name: 'lact',allowBlank:false,stateful:false,anchor:'95%',keepZero:false}]},
	...
	save: function(){
        ...
        keyparams[1]={ColumnName:'letm',ColumnValue:this.basisForm.findField('letm').getValue(),ColumnType:'int'};
        
	    var method=this.isNew?'new':'edit';
		Ext.Ajax.request({
		   url: ContextInfo.contextPath+'/' + atshflatPageName + '.mvc/'+method,
...
var atshflatPanel=function(){
    this.tabId=atshflatConfig.tabId;
	this.init();	
	
	atshflatPanel.superclass.constructor.call(this,{
		applyTo:this.tabId, 
	    layout:'fit',
	    ...
	    items:[{
	        ...
	        tbar: [{ 
	            ...
	            hidden:atshflatConfig.auth[this.tabId+'_' + 'add']!='True',
	            handler: function(){
	                new atshflatEditWindow(this.grid,true).show();            	
	            }, 
	            scope: this 
	        },'-',{ 
	        	id:this.tabId+ '_edit',
	            ...
	            hidden:atshflatConfig.auth[this.tabId+'_' + 'edit']!='True',
	            handler: function(){
	            	new atshflatEditWindow(this.grid).show(); 
	            }, 
	            scope: this
	        },{ 
	            id:this.tabId+ '_delete',
                iconCls:'icon-remove', 
                hidden:atshflatConfig.auth[this.tabId+'_' + 'delete']!='True',
	            ...
	            iconCls:'icon-query', 
	            text:HRMSRes.Public_Toolbar_Query,
	            hidden:atshflatConfig.auth[this.tabId+'_' + 'query']!='True',
	            handler: function(){
	            	new atshflatQueryWindow(this.grid).show();
	            }, 
	            scope: this
	        },'-',{ 
	            ...
	            hidden:atshflatConfig.auth[this.tabId+'_' + 'exportexcel']!='True',
                ...
                id:this.tabId+'_muf',
                iconCls:atshflatConfig.muf=='True'?'icon-mufdelete':'icon-mufadd', 
                text:'', 
                handler: function(){updateMUF(this.tabId,atshflatConfig.muf?'delete':'add',atshflatConfig,this.grid);},
                scope: this 
            }], 
	        items:this.grid
       }]
...
Ext.extend(atshflatPanel,Ext.Panel,{
    init:function(){
	...
	createGridPanel:function(){
		...
		var atshflatStoreType=Ext.data.Record.create([
            {name:'sfcd'},
            {name:'sfnm'},
            {name:'letm'},
            {name:'lact'},
		...
		var store=new Ext.data.Store({ 
        	reader: new Ext.data.JsonReader({
	   	    ...
	   	    baseParams:{record:Ext.encode({params:[{ColumnName:atshflatKeyColumn,ColumnValue:atshflatConfig.sfcd}]})},   
	   		url:ContextInfo.contextPath+'/' + atshflatPageName + '.mvc/list',
	   		listeners:{
        ...
        return new Ext.grid.GridPanel({
            ...
            cm: new Ext.grid.ColumnModel([ 
                ...
                {header:HRMSRes.Attendance_Label_letm,sortable: true, dataIndex: 'letm'},
	...
	remove:function(){
        ...
        keyparams[1]={ColumnName:'letm',ColumnValue:record.get('letm'),ColumnType:'int'};
	    ...
	    Ext.MessageBox.show({
	        ...
	        fn: function(btn, text){
		        if (btn=='yes'){
			        Ext.Ajax.request({
	   			        url:ContextInfo.contextPath+ '/'+ atshflatPageName + '.mvc/delete',
	   			        success: function(response){
	   				        var o= Ext.util.JSON.decode(response.responseText);
	   				        if(o.status=='success'){
	...
	exportExcel:function(){
	    ...
	    var params={record:Ext.encode({params:[{ColumnName:atshflatKeyColumn,ColumnValue:''}],headers:header})};
	    ...
	    form.name='excelForm';
	    form.method='post';
	    form.action=ContextInfo.contextPath+ '/' + atshflatPageName + '.mvc/exportexcel';
	    for(var p in params){
