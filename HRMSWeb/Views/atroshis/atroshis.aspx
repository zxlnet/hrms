<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="atroshis.aspx.cs" Inherits="GotWell.HRMS.HRMSWeb.Views.atroshis.atroshis" %>
...
var atroshisConfig=Ext.decode('<%=ViewData["config"] %>'); 
atroshisConfig.auth=Ext.decode('<%=ViewData["authorization"] %>');
var atroshisPageName = 'atroshis';
var atroshisKeyColumn='emno';
            
var atroshisQueryWindow=function(grid){
	this.grid=grid;
	this.store=this.grid.getStore();
	this.init();	
	
	atroshisQueryWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:400, 
        height:270, 
        closeAction:'close', 
...
Ext.extend(atroshisQueryWindow,Ext.Window,{
    init:function(){
	...
	createFormPanel:function(){	        
		return new Ext.FormPanel({   
             ...
             items: [
                    ...
                    {columnWidth:.5,layout: 'form',
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Attendance_Label_rscd ,
...
var atroshisEditWindow=function(grid,isNew){
	this.grid=grid;
	this.isNew=isNew||false;
	this.store=this.grid.getStore();
	this.init();	

	atroshisEditWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:650, 
        height:350, 
        ...
        listeners:{
            show:function(){              
		        var keyField = this.basisForm.findField(atroshisKeyColumn);

                if(!this.isNew){	
			        var data=this.grid.getSelectionModel().getSelected();
                    this.basisForm.items.each(function(f){
                        if(f.isFormField){
                            var value=data.get(f.getName());if (f.xtype=='timefield') value = formatTime(value);
		        ...
		        {
		            keyField.setValue(atroshisConfig.emno);
		            if (atroshisConfig.emno!='')
		                getMaxsqno(atroshisConfig.emno,atroshisConfig.tableName,this.basisForm);
		        }
		        var keyValue = keyField.getValue();
                setLastModifiedInfo(atroshisConfig,this.basisForm);
            },
            scope:this
        },
        buttons: [{ 
...
Ext.extend(atroshisEditWindow,Ext.Window,{
    init:function(){
	         ...
	         header:true,
      		 items: [
      		    ...
      		    items:[
                    ...
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Public_Label_staff + '(<font color=red>*</font>)',
                        ...
                        store: new Ext.data.Store({ 
		                    ...
		                    listeners:{
                                ...
                                scope:this}}),		                    
                            listeners:{
                                select:function(p){var emno = p.getValue();if (emno!='') {getMaxsqno(emno,atroshisConfig.tableName,this.basisForm);}},
    			                scope:this}
    		          }]},
                    ...
                    {columnWidth:.5,layout: 'form',
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Attendance_Label_iaod ,
                        name: 'iaod',stateful:false,editable:false,typeAhead: true,
                        triggerAction: 'all',mode: 'local',maxHeight:150,anchor:'95%',
	...
	save: function(){
		...
		Ext.Ajax.request({
		   url: ContextInfo.contextPath+'/' + atroshisPageName + '.mvc/'+method,
	...
	applyto: function(){
        ...
        var url = ContextInfo.contextPath+'/' + atroshisPageName + '.mvc/applyto';
        new empAdvQryQueryWindow(this.grid,'1',url,params).show();
	}
});


var atroshisPanel=function(){
    this.tabId=atroshisConfig.tabId;
	this.init();	
	
	atroshisPanel.superclass.constructor.call(this,{
		applyTo:this.tabId, 
	    layout:'fit',
	    ...
	    items:[{
	        ...
	        tbar: [{ 
	            ...
	            hidden:atroshisConfig.auth[this.tabId+'_' + 'add']!='True',
	            handler: function(){
	                new atroshisEditWindow(this.grid,true).show();            	
	            }, 
	            scope: this 
	        },'-',{ 
	        	id:this.tabId+ '_edit',
	            ...
	            hidden:atroshisConfig.auth[this.tabId+'_' + 'edit']!='True',
	            handler: function(){
	            	new atroshisEditWindow(this.grid).show(); 
	            }, 
	            scope: this
	        },{ 
	            id:this.tabId+ '_delete',
                iconCls:'icon-remove', 
                hidden:atroshisConfig.auth[this.tabId+'_' + 'delete']!='True',
	            ...
	            iconCls:'icon-query', 
	            text:HRMSRes.Public_Toolbar_Query,
	            hidden:atroshisConfig.auth[this.tabId+'_' + 'query']!='True',
	            handler: function(){
	            	new atroshisQueryWindow(this.grid).show();
	            }, 
	            scope: this
	        },'-',{ 
	            ...
	            hidden:atroshisConfig.auth[this.tabId+'_' + 'exportexcel']!='True',
	            handler: this.exportExcel, 
	            scope: this 
	        },'->',{ 
	        	xtype:'label', 
	            html:  '<b><font color=green>employee no: ' + atroshisConfig.emno + '</font></b>',
	            hidden: atroshisConfig.emno==''
	        },'->',{ 
                ...
                id:this.tabId+'_muf',
                iconCls:atroshisConfig.muf=='True'?'icon-mufdelete':'icon-mufadd', 
                text:'', 
                handler: function(){updateMUF(this.tabId,atroshisConfig.muf?'delete':'add',atroshisConfig,this.grid);},
                scope: this 
            }], 
	        items:this.grid
       }]
...
Ext.extend(atroshisPanel,Ext.Panel,{
    init:function(){
	...
	createGridPanel:function(){
		...
		var atroshisStoreType=Ext.data.Record.create([
		    {name:'emno'},
            {name:'sfid'},
            {name:'stfn'},
            {name:'sqno'},
		...
		var store=new Ext.data.Store({ 
        	reader: new Ext.data.JsonReader({
	   	    ...
	   	    baseParams:{record:Ext.encode({params:[{ColumnName:atroshisKeyColumn,ColumnValue:atroshisConfig.emno}]})},   
	   		url:ContextInfo.contextPath+'/' + atroshisPageName + '.mvc/list',
	   		listeners:{
        ...
        return new Ext.grid.GridPanel({
            ...
            cm: new Ext.grid.ColumnModel([ 
                ...
                {header:HRMSRes.Attendance_Label_iaod,sortable: true, dataIndex: 'iaod'},
	...
	remove:function(){
	    ...
	    Ext.MessageBox.show({
	        ...
	        fn: function(btn, text){
		        if (btn=='yes'){
			        Ext.Ajax.request({
	   			        url:ContextInfo.contextPath+ '/'+ atroshisPageName + '.mvc/delete',
	   			        success: function(response){
	   				        var o= Ext.util.JSON.decode(response.responseText);
	   				        if(o.status=='success'){
	...
	exportExcel:function(){
	    ...
	    var params={record:Ext.encode({params:[{ColumnName:atroshisKeyColumn,ColumnValue:''}],headers:header})};
	    ...
	    form.name='excelForm';
	    form.method='post';
	    form.action=ContextInfo.contextPath+ '/' + atroshisPageName + '.mvc/exportexcel';
	    for(var p in params){
