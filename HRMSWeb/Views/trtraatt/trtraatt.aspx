<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="trtraatt.aspx.cs" Inherits="GotWell.HRMS.HRMSWeb.Views.trtraatt.trtraatt" %>
...
var trtraattConfig=Ext.decode('<%=ViewData["config"] %>'); 
trtraattConfig.auth=Ext.decode('<%=ViewData["authorization"] %>');
var trtraattPageName = 'trtraatt';
var trtraattKeyColumn='trcd';
            
var trtraattQueryWindow=function(grid){
	this.grid=grid;
	this.store=this.grid.getStore();
	this.init();	
	
	trtraattQueryWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:400, 
        height:270, 
        closeAction:'close', 
...
Ext.extend(trtraattQueryWindow,Ext.Window,{
    init:function(){
...
var trtraattEditWindow=function(grid,isNew){
	this.grid=grid;
	this.isNew=isNew||false;
	this.store=this.grid.getStore();
	this.init();	

	trtraattEditWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:650, 
        height:350, 
        ...
        listeners:{
            show:function(){              
		        var keyField = this.basisForm.findField(trtraattKeyColumn);
                ...
                setLastModifiedInfo(trtraattConfig,this.basisForm);
            },
            scope:this
        },
        buttons: [{ 
...
Ext.extend(trtraattEditWindow,Ext.Window,{
    init:function(){
      		    ...
      		    items:[
                    ...
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Personal_Label_trcd,
                        ...
                        store: new Ext.data.Store({ 
		                    ...
		                    listeners:{load: function(){var v = this.basisForm.findField('trcd').getValue();this.basisForm.findField('trcd').setValue(v);},scope:this
		                    }})
    		          }]},
                    ...
                    {columnWidth:.5,layout: 'form',
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Training_Label_isrg ,
                        name: 'isrg',stateful:false,editable:false,typeAhead: true,
                        triggerAction: 'all',mode: 'local',maxHeight:150,anchor:'95%',
                        displayField: 'text',valueField:'value',
                        store: FlagYesNoStore}]},

     		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'datefield',fieldLabel:HRMSRes.Training_Label_rgtm,id:'rgtm',
                        name:'rgtm',editable:false,height:22,anchor:'95%',
                        format: DATE_FORMAT.DATETIME,minValue: '1980/01/01',stateful:false,
           	 	        invalidText : '{0} '+ HRMSRes.Public_Messsage_FormatInCorrect +' {1}'}]},

       		        {columnWidth:.5,layout: 'form',
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Training_Label_rgur,
                        name: 'rgur',stateful:false,anchor:'95%'}]},

                    {columnWidth:.5,layout: 'form',
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Training_Label_isat ,
                        name: 'isat',stateful:false,editable:false,typeAhead: true,
                        triggerAction: 'all',mode: 'local',maxHeight:150,anchor:'95%',
                        displayField: 'text',valueField:'value',
                        store: FlagYesNoStore}]},

//      		        {columnWidth:.5,layout: 'form',
//                     items: [{xtype:'numberfield',fieldLabel:HRMSRes.Training_Label_abrs,
//                        name: 'abrs',stateful:false,anchor:'95%',decimalPrecision:0,keepZero:false}]},
	...
	save: function(){
        ...
        keyparams[1]={ColumnName:'emno',ColumnValue:this.basisForm.findField('emno').getValue()};
        
	    var method=this.isNew?'new':'edit';
		Ext.Ajax.request({
		   url: ContextInfo.contextPath+'/' + trtraattPageName + '.mvc/'+method,
...
var trtraattPanel=function(){
    this.tabId=trtraattConfig.tabId;
	this.init();	
	
	trtraattPanel.superclass.constructor.call(this,{
		applyTo:this.tabId, 
	    layout:'fit',
	    ...
	    items:[{
	        ...
	        tbar: [{ 
	            ...
	            hidden:trtraattConfig.auth[this.tabId+'_' + 'add']!='True',
	            handler: function(){
	                new trtraattEditWindow(this.grid,true).show();            	
	            }, 
	            scope: this 
	        },'-',{ 
	        	id:this.tabId+ '_edit',
	            ...
	            hidden: trtraattConfig.auth[this.tabId + '_' + 'edit'] != 'True',
	            disabled:true,
	            handler: function(){
	            	new trtraattEditWindow(this.grid).show(); 
	            }, 
	            scope: this
	        },{ 
	            id:this.tabId+ '_delete',
                iconCls:'icon-remove',
                hidden: trtraattConfig.auth[this.tabId + '_' + 'delete'] != 'True',
                disabled:true,
	            ...
	            iconCls:'icon-query', 
	            text:HRMSRes.Public_Toolbar_Query,
	            hidden:trtraattConfig.auth[this.tabId+'_' + 'query']!='True',
	            handler: function(){
	            	new trtraattQueryWindow(this.grid).show();
	            }, 
	            scope: this
	        },'-',{ 
	            ...
	            hidden:trtraattConfig.auth[this.tabId+'_' + 'exportexcel']!='True',
                ...
                id:this.tabId+'_muf',
                iconCls:trtraattConfig.muf=='True'?'icon-mufdelete':'icon-mufadd', 
                text:'', 
                handler: function(){updateMUF(this.tabId,trtraattConfig.muf?'delete':'add',trtraattConfig,this.grid);},
                scope: this 
            }], 
	        items:this.grid
       }]
...
Ext.extend(trtraattPanel,Ext.Panel,{
    init:function(){
	...
	createGridPanel:function(){
		...
		var trtraattStoreType=Ext.data.Record.create([
		    {name:'sfid'},
		    {name:'stfn'},
            {name:'trcd'},
            {name:'emno'},
		...
		var store=new Ext.data.Store({ 
        	reader: new Ext.data.JsonReader({
	   	    ...
	   	    baseParams:{record:Ext.encode({params:[{ColumnName:trtraattKeyColumn,ColumnValue:trtraattConfig.trcd}]})},   
	   		url:ContextInfo.contextPath+'/' + trtraattPageName + '.mvc/list',
	   		listeners:{
        ...
        return new Ext.grid.GridPanel({
            ...
            cm: new Ext.grid.ColumnModel([ 
                ...
                {header:HRMSRes.Training_Label_trnm,sortable: true, dataIndex: 'trnm',width:250},
                {header:HRMSRes.Training_Label_isrg,sortable: true, dataIndex: 'isrg'},
                {header:HRMSRes.Training_Label_isat,sortable: true, dataIndex: 'isat'},
	...
	remove:function(){
	    ...
	    Ext.MessageBox.show({
	        ...
	        fn: function(btn, text){
		        if (btn=='yes'){
			        Ext.Ajax.request({
	   			        url:ContextInfo.contextPath+ '/'+ trtraattPageName + '.mvc/delete',
	   			        success: function(response){
	   				        var o= Ext.util.JSON.decode(response.responseText);
	   				        if(o.status=='success'){
	...
	exportExcel:function(){
	    ...
	    var params={record:Ext.encode({params:[{ColumnName:trtraattKeyColumn,ColumnValue:''}],headers:header})};
	    ...
	    form.name='excelForm';
	    form.method='post';
	    form.action=ContextInfo.contextPath+ '/' + trtraattPageName + '.mvc/exportexcel';
	    for(var p in params){
