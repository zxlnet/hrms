<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="prcstalc.aspx.cs" Inherits="GotWell.HRMS.HRMSWeb.Views.prcstalc.prcstalc" %>
...
var prcstalcConfig=Ext.decode('<%=ViewData["config"] %>'); 
prcstalcConfig.auth=Ext.decode('<%=ViewData["authorization"] %>');
var prcstalcPageName = 'prcstalc';
var prcstalcKeyColumn='emno';
            
var prcstalcQueryWindow=function(grid){
	this.grid=grid;
	this.store=this.grid.getStore();
	this.init();	
	
	prcstalcQueryWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:400, 
        height:220, 
        closeAction:'close', 
...
Ext.extend(prcstalcQueryWindow,Ext.Window,{
    init:function(){
...
Ext.extend(calculationStatusWindow,Ext.Window,{
	...
	calculation:function(){
		Ext.Ajax.request({
		   url: ContextInfo.contextPath+'/' + prcstalcPageName + '.mvc/calculation',
...
var prcstalcPanel=function(){
    this.tabId=prcstalcConfig.tabId;
	this.init();	
	
	prcstalcPanel.superclass.constructor.call(this,{
		applyTo:this.tabId, 
	    layout:'fit',
	    ...
	    items:[{
	        ...
	        tbar: [{ 
	            ...
	            //hidden:prcstalcConfig.auth[this.tabId+'_' + 'calculate']!='True',
	            handler: function(){
	            ...
	            iconCls:'icon-query', 
	            text:HRMSRes.Public_Toolbar_Query,
	            hidden:prcstalcConfig.auth[this.tabId+'_' + 'query']!='True',
	            handler: function(){
	            	new prcstalcQueryWindow(this.grid).show();
	            }, 
	            scope: this
	        },'-',{ 
	            ...
	            hidden:prcstalcConfig.auth[this.tabId+'_' + 'exportexcel']!='True',
                ...
                id:this.tabId+'_muf',
                iconCls:prcstalcConfig.muf=='True'?'icon-mufdelete':'icon-mufadd', 
                text:'', 
                handler: function(){updateMUF(this.tabId,prcstalcConfig.muf?'delete':'add',prcstalcConfig,this.grid);},
                scope: this 
            }], 
	        items:this.grid
       }]
...
Ext.extend(prcstalcPanel,Ext.Panel,{
    init:function(){
	...
	createGridPanel:function(){
	    ...
	    var prcstalcStoreType=Ext.data.Record.create([
		    {name:'emno'},
            {name:'sfid'},
            {name:'sfnm'},
            {name:'perd'},
		...
		var store=new Ext.data.Store({ 
        	reader: new Ext.data.JsonReader({
	   	    ...
	   	    baseParams:{record:Ext.encode({params:[{ColumnName:prcstalcKeyColumn,ColumnValue:prcstalcConfig.emno}]})},   
	   		url:ContextInfo.contextPath+'/' + prcstalcPageName + '.mvc/listdetails',
	   		listeners:{
    ...
    LoadDetailsGrid:function(grid){
        var store;
        var prcstalc_SummaryType=Ext.data.Record.create([
		    {name:'emno'},
            {name:'sfid'},
            {name:'sfnm'},
            {name:'perd'},
	...
	LoadSummaryGrid:function(grid){
        var store;
        var prcstalc_DetailsType=Ext.data.Record.create([
		    {name:'emno'},
            {name:'sfid'},
            {name:'sfnm'},
            {name:'perd'},
        ...
        store=new Ext.data.Store({ 
            reader: new Ext.data.JsonReader({
                totalProperty: "results",
                root: "rows"               
            },prcstalc_DetailsType), 		    
            url:ContextInfo.contextPath+'/' + prcstalcPageName + '.mvc/listsummary',
            listeners:
            {
                loadexception:function(o,t,response)
                {
	...
	exportExcel:function(){
	    ...
	    var params={record:Ext.encode({params:[{ColumnName:prcstalcKeyColumn,ColumnValue:''}],headers:header})};
	    ...
	    form.name='excelForm';
	    form.method='post';
	    form.action=ContextInfo.contextPath+ '/' + prcstalcPageName + '.mvc/exportexcel';
	    for(var p in params){
