<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="prprirst.aspx.cs" Inherits="GotWell.HRMS.HRMSWeb.Views.prprirst.prprirst" %>
...
var prprirstConfig=Ext.decode('<%=ViewData["config"] %>'); 
prprirstConfig.auth=Ext.decode('<%=ViewData["authorization"] %>');
var prprirstPageName = 'prprirst';
var prprirstKeyColumn='emno';
            
var prprirstQueryWindow=function(grid){
	this.grid=grid;
	this.store=this.grid.getStore();
	this.init();	
	
	prprirstQueryWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:400, 
        height:270, 
        closeAction:'close', 
...
Ext.extend(prprirstQueryWindow,Ext.Window,{
    init:function(){
...
Ext.extend(prrelItemEditWindow,Ext.Window,{
    ...
    createGridPanel:function(){
		var prprirstDtlStoreType=Ext.data.Record.create([
		    {name:'sele'},
            {name:'sqno'},
            {name:'itcd'},
            {name:'itnm'},
...
var prprirstDtlEditWindow=function(rec,store,flag){
	this.feedbackStore=store;
	this.isNewDtl = flag;
	this.feedbackRec = rec;
	this.init();	

	prprirstDtlEditWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:650, 
        height:300, 
        ...
        listeners:{
            show:function(){              
                ...
                setLastModifiedInfo(prprirstConfig,this.basisDtlEditForm);
            },
            scope:this
        },
        buttons: [{ 
...
Ext.extend(prprirstDtlEditWindow,Ext.Window,{
    init:function(){
...
//                        name: 'cred',stateful:false,anchor:'95%',hidden:true,hideLabel:true}]},
...
var prprirstEditWindow=function(grid,isNew){
	this.grid=grid;
	...
	prprirstEditWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:750, 
        height:470, 
        ...
        listeners:{
            show:function(){    
		        var keyField = this.basisForm.findField(prprirstKeyColumn);
                if(!this.isNew){	
...
Ext.extend(prprirstEditWindow,Ext.Window,{
    init:function(){
    ...
    createGridPanel:function(){
		...
		var store=new Ext.data.Store({ 
        	reader: new Ext.data.JsonReader({
	   	    ...
	   	    baseParams:{record:Ext.encode({params:[{ColumnName:prprirstKeyColumn,ColumnValue:''}]})},   
	   		url:ContextInfo.contextPath+'/' + prprirstPageName + '.mvc/getPriRuleSetDtl',
	   		listeners:{
                loadexception:function(o,t,response)
                {
                    var o= Ext.util.JSON.decode(response.responseText);		   		
...
//        });
		...
		Ext.Ajax.request({
		   url: ContextInfo.contextPath+'/' + prprirstPageName + '.mvc/'+method,
	...
	Query:function(){	    
        var params=[];	
        var f = this.basisForm.findField(prprirstKeyColumn);        
        var p={
            ColumnName:f.getName(),
            ColumnValue:f.getValue()                
        };
    ...
    addline:function(){	    
        ...
        params['lmur']=prprirstConfig.currentUser;

        var store = this.dtlgrid.getStore();
        new prprirstDtlEditWindow(new store.recordType(params),store,true).show();
    },
    editline:function(){	    
        var n=this.dtlgrid.getStore().getCount() + 1;
        var rec = this.dtlgrid.getSelectionModel().getSelected();
        var store = this.dtlgrid.getStore();    
        new prprirstDtlEditWindow(rec,store,false).show();
    },
	deleteline:function(){
	...
	exportExcelLine:function(){
	    ...
	    var params={record:Ext.encode({params:[{ColumnName:prprirstKeyColumn,ColumnValue:''}],headers:header})};
	    ...
	    form.name='excelForm';
	    form.method='post';
	    form.action=ContextInfo.contextPath+ '/' + prprirstPageName + '.mvc/exportexceldtl';
	    for(var p in params){
    ...
    applyto:function(){
        ...
        var url = ContextInfo.contextPath+'/' + prprirstPageName + '.mvc/applyto';
        new empAdvQryQueryWindow(this.dtlgrid,'1',url,dtlparams).show();
    }    
});


var prprirstPanel=function(){
    this.tabId=prprirstConfig.tabId;
	this.init();	
	
	prprirstPanel.superclass.constructor.call(this,{
		applyTo:this.tabId, 
	    layout:'fit',
...
//	            iconCls:'icon-add', 
//	            text: HRMSRes.Public_Toolbar_Add, 
//	            hidden:prprirstConfig.auth[this.tabId+'_' + 'add']!='True',
//	            handler: function(){
//	                new prprirstEditWindow(this.grid,true).show();            	
//	            }, 
//	            scope: this 
//	        },'-',
	        ...
	        { 
	            ...
	            iconCls:'icon-update', 
	            text:HRMSRes.Public_Toolbar_Edit,
	            hidden: prprirstConfig.auth[this.tabId + '_' + 'edit'] != 'True',
	            disabled:true,
	            handler: function(){
	            	new prprirstEditWindow(this.grid).show(); 
	            }, 
	            scope: this
	        },
//	        { 
...
//                iconCls:'icon-remove', 
//                hidden:prprirstConfig.auth[this.tabId+'_' + 'delete']!='True',
//                text:HRMSRes.Public_Toolbar_Delete, 
//                handler: this.remove, 
//                scope: this 
//            },
            ...
            '-',{ 
	            ...
	            iconCls:'icon-query', 
	            text:HRMSRes.Public_Toolbar_Query,
	            hidden:prprirstConfig.auth[this.tabId+'_' + 'query']!='True',
	            handler: function(){
	            	new prprirstQueryWindow(this.grid).show();
	            }, 
	            scope: this
	        },'-',{ 
	            ...
	            iconCls:'icon-export', 
	            text:HRMSRes.Public_Toolbar_ToExcel, 
	            hidden:prprirstConfig.auth[this.tabId+'_' + 'exportexcel']!='True',
                ...
                id:this.tabId+'_muf',
                iconCls:prprirstConfig.muf=='True'?'icon-mufdelete':'icon-mufadd', 
                text:'', 
                handler: function(){updateMUF(this.tabId,prprirstConfig.muf?'delete':'add',prprirstConfig,this.grid);},
                scope: this 
            }], 
	        items:this.grid
       }]
...
Ext.extend(prprirstPanel,Ext.Panel,{
    init:function(){
	...
	createGridPanel:function(){
		...
		var prprirstStoreType=Ext.data.Record.create([
		...
		var store=new Ext.data.Store({ 
        	reader: new Ext.data.JsonReader({
	   	    ...
	   	    baseParams:{record:Ext.encode({params:[{ColumnName:prprirstKeyColumn,ColumnValue:prprirstConfig.emno}]})},   
	...
	remove:function(){
	    ...
	    Ext.MessageBox.show({
	        ...
	        fn: function(btn, text){
		        if (btn=='yes'){
			        Ext.Ajax.request({
	   			        url:ContextInfo.contextPath+ '/'+ prprirstPageName + '.mvc/delete',
	   			        success: function(response){
	   				        var o= Ext.util.JSON.decode(response.responseText);
	   				        if(o.status=='success'){
