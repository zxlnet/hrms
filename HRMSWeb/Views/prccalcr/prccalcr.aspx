<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="prccalcr.aspx.cs" Inherits="GotWell.HRMS.HRMSWeb.Views.prccalcr.prccalcr" %>
...
var prccalcrConfig=Ext.decode('<%=ViewData["config"] %>'); 
prccalcrConfig.auth=Ext.decode('<%=ViewData["authorization"] %>');
var prccalcrPageName = 'prccalcr';
var prccalcrKeyColumn='emno';
            
var prccalcrQueryWindow=function(grid){
	this.grid=grid;
	this.store=this.grid.getStore();
	this.init();	
	
	prccalcrQueryWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:400, 
        height:310, 
        closeAction:'close', 
...
Ext.extend(prccalcrQueryWindow,Ext.Window,{
    init:function(){
...
var prccalcrEditWindow=function(grid,isNew){
	this.grid=grid;
	this.isNew=isNew||false;
	this.store=this.grid.getStore();
	this.init();	
                     
	prccalcrEditWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:650, 
        height:350, 
        ...
        listeners:{
            show:function(){              
		        var keyField = this.basisForm.findField(prccalcrKeyColumn);
		        ...
		        {
		            keyField.setValue(prccalcrConfig.emno);
		            if (prccalcrConfig.emno!='')
		                getMaxsqno(prccalcrConfig.emno,prccalcrConfig.tableName,this.basisForm);
		        }
		        var keyValue = keyField.getValue();
                setLastModifiedInfo(prccalcrConfig,this.basisForm);
            },
            scope:this
        },
        buttons: [{ 
...
Ext.extend(prccalcrEditWindow,Ext.Window,{
    init:function(){
	         ...
	         header:true,
      		 items: [{
      		    ...
      		    items:[
                    ...
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Public_Label_staff + '(<font color=red>*</font>)',
                        ...
                        store: new Ext.data.Store({ 
                            ...
                            listeners:{select:function(p){
                                var emno = p.getValue();
                                if (emno!=''){
                                    getMaxsqno(emno,prccalcrConfig.tableName,this.basisForm);
                                }
                            },scope:this}
    		          }]},
                    ...
                    {columnWidth:.5,layout: 'form',
                    items: [{xtype:'combo',fieldLabel:HRMSRes.Master_Label_ccnm +'(<font color=red>*</font>)',
                        name: 'cccd',stateful:false,typeAhead: true,allowBlank:false,
                        triggerAction: 'all',mode: 'local',maxHeight:150,anchor:'95%',
                        ...
                        store: new Ext.data.Store({ 
		                    ...
		                    listeners:{load:function(){var f = this.basisForm.findField('cccd');f.setValue(f.getValue());},scope:this}})
    		          }]},
	...
	save: function(){
		...
		Ext.Ajax.request({
		   url: ContextInfo.contextPath+'/' + prccalcrPageName + '.mvc/'+method,
...
var prccalcrPanel=function(){
    this.tabId=prccalcrConfig.tabId;
	this.init();	
	
	prccalcrPanel.superclass.constructor.call(this,{
		applyTo:this.tabId, 
	    layout:'fit',
	    ...
	    items:[{
	        ...
	        tbar: [{ 
	            ...
	            hidden:prccalcrConfig.auth[this.tabId+'_' + 'add']!='True',
	            handler: function(){
	                new prccalcrEditWindow(this.grid,true).show();            	
	            }, 
	            scope: this 
	        },'-',{ 
	        	id:this.tabId+ '_edit',
	            ...
	            hidden:prccalcrConfig.auth[this.tabId+'_' + 'edit']!='True',
	            handler: function(){
	            	new prccalcrEditWindow(this.grid).show(); 
	            }, 
	            scope: this
	        },{ 
	            id:this.tabId+ '_delete',
                iconCls:'icon-remove', 
                hidden:prccalcrConfig.auth[this.tabId+'_' + 'delete']!='True',
	            ...
	            iconCls:'icon-query', 
	            text:HRMSRes.Public_Toolbar_Query,
	            hidden:prccalcrConfig.auth[this.tabId+'_' + 'query']!='True',
	            handler: function(){
	            	new prccalcrQueryWindow(this.grid).show();
	            }, 
	            scope: this
	        },'-',{ 
	            ...
	            hidden:prccalcrConfig.auth[this.tabId+'_' + 'exportexcel']!='True',
	            handler: this.exportExcel, 
	            scope: this 
	        },'->',{ 
	        	xtype:'label', 
	            html:  '<b><font color=green>employee no: ' + prccalcrConfig.emno + '</font></b>',
	            hidden: prccalcrConfig.emno==''
	        },'->',{ 
                ...
                id:this.tabId+'_muf',
                iconCls:prccalcrConfig.muf=='True'?'icon-mufdelete':'icon-mufadd', 
                text:'', 
                handler: function(){updateMUF(this.tabId,prccalcrConfig.muf?'delete':'add',prccalcrConfig,this.grid);},
                scope: this 
            }], 
	        items:this.grid
       }]
...
Ext.extend(prccalcrPanel,Ext.Panel,{
    init:function(){
	...
	createGridPanel:function(){
		...
		var prccalcrStoreType=Ext.data.Record.create([
		    {name:'emno'},
            {name:'sfid'},
            {name:'stfn'},
            {name:'sqno'},
		...
		var store=new Ext.data.Store({ 
        	reader: new Ext.data.JsonReader({
	   	    ...
	   	    baseParams:{record:Ext.encode({params:[{ColumnName:prccalcrKeyColumn,ColumnValue:prccalcrConfig.emno}]})},   
	   		url:ContextInfo.contextPath+'/' + prccalcrPageName + '.mvc/list',
	   		listeners:{
        ...
        return new Ext.grid.GridPanel({
            ...
            cm: new Ext.grid.ColumnModel([ 
                ...
                {header:HRMSRes.Master_Label_ccnm,sortable: true, dataIndex: 'ccnm'},
	...
	remove:function(){
	    ...
	    Ext.MessageBox.show({
	        ...
	        fn: function(btn, text){
		        if (btn=='yes'){
			        Ext.Ajax.request({
	   			        url:ContextInfo.contextPath+ '/'+ prccalcrPageName + '.mvc/delete',
	   			        success: function(response){
	   				        var o= Ext.util.JSON.decode(response.responseText);
	   				        if(o.status=='success'){
	...
	exportExcel:function(){
	    ...
	    var params={record:Ext.encode({params:[{ColumnName:prccalcrKeyColumn,ColumnValue:''}],headers:header})};
	    ...
	    form.name='excelForm';
	    form.method='post';
	    form.action=ContextInfo.contextPath+ '/' + prccalcrPageName + '.mvc/exportexcel';
	    for(var p in params){
