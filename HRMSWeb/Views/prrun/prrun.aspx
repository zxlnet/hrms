<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="prrun.aspx.cs" Inherits="GotWell.HRMS.HRMSWeb.Views.prrun.prrun" %>
...
var prrunConfig=Ext.decode('<%=ViewData["config"] %>'); 
prrunConfig.auth=Ext.decode('<%=ViewData["authorization"] %>');
var prrunPageName = 'prrun';
var prrunKeyColumn='rnno';
            
var prrunQueryWindow=function(grid){
	this.grid=grid;
	this.store=this.grid.getStore();
	this.init();	
	
	prrunQueryWindow.superclass.constructor.call(this,{
        layout:'fit', 
        width:400, 
        height:270, 
        closeAction:'close', 
...
Ext.extend(prrunQueryWindow,Ext.Window,{
    init:function(){
		...
		return new Ext.FormPanel({   
             ...
             items: [
                     ...
                     items: [{xtype:'textfield',fieldLabel:HRMSRes.Payroll_Label_rnno,
                        name: 'rnno',stateful:false,anchor:'95%',UpperOnBlur:'true'}]},
...
var prrunPanel=function(){
    this.tabId=prrunConfig.tabId;
	this.init();	
	
	prrunPanel.superclass.constructor.call(this,{
		applyTo:this.tabId, 
	    layout:'fit',
	    ...
	    items:[{
	        ...
	        tbar: [{ 
                ...
                iconCls:'icon-remove', 
                hidden:prrunConfig.auth[this.tabId+'_' + 'delete']!='True',
	            ...
	            iconCls:'icon-query', 
	            text:HRMSRes.Public_Toolbar_Query,
	            hidden:prrunConfig.auth[this.tabId+'_' + 'query']!='True',
	            handler: function(){
	            	new prrunQueryWindow(this.grid).show();
	            }, 
	            scope: this
	        },'-',{ 
	            ...
	            hidden:prrunConfig.auth[this.tabId+'_' + 'exportexcel']!='True',
	            handler: this.exportExcel, 
	            scope: this 
	        },'->',{ 
                id:this.tabId+'_muf',
                iconCls:prrunConfig.muf=='True'?'icon-mufdelete':'icon-mufadd', 
                text:'', 
                handler: function(){updateMUF(this.tabId,prrunConfig.muf?'delete':'add',prrunConfig,this.grid);},
                scope: this 
            }], 
	        items:this.grid
       }]
...
Ext.extend(prrunPanel,Ext.Panel,{
    init:function(){
	...
	createGridPanel:function(){
		...
		var prrunStoreType=Ext.data.Record.create([
		    {name:'rnno'},
            {name:'rndt'},
            {name:'rnty'},
            {name:'tamt'},
		...
		var store=new Ext.data.Store({ 
        	reader: new Ext.data.JsonReader({
	   	    ...
	   	    baseParams:{record:Ext.encode({params:[{ColumnName:prrunKeyColumn,ColumnValue:''}]})},   
	   		url:ContextInfo.contextPath+'/' + prrunPageName + '.mvc/list',
	   		listeners:{
        ...
        return new Ext.grid.GridPanel({
            ...
            cm: new Ext.grid.ColumnModel([ 
                ...
                {header:HRMSRes.Payroll_Label_rndt,sortable: true, dataIndex: 'rndt',renderer:formatDateNoTime},
                {header:HRMSRes.Payroll_Label_rnty,sortable: true, dataIndex: 'rnty'},
                {header:HRMSRes.Payroll_Label_tmat,sortable: true, dataIndex: 'tamt'},
                {header:HRMSRes.Payroll_Label_tsct,sortable: true, dataIndex: 'tsct'},
	...
	remove:function(){
		...
		var keyparams=[];
        keyparams[0]={ColumnName:'rnno',ColumnValue:record.get('rnno')};
	    ...
	    Ext.MessageBox.show({
	        ...
	        fn: function(btn, text){
		        if (btn=='yes'){
			        Ext.Ajax.request({
	   			        url:ContextInfo.contextPath+ '/'+ prrunPageName + '.mvc/delete',
	...
	exportExcel:function(){
	    ...
	    var params={record:Ext.encode({params:[{ColumnName:prrunKeyColumn,ColumnValue:''}],headers:header})};
	    ...
	    form.name='excelForm';
	    form.method='post';
	    form.action=ContextInfo.contextPath+ '/' + prrunPageName + '.mvc/exportexcel';
	    for(var p in params){
